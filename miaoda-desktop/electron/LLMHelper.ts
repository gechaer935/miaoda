...
import { autoLanguageFallbackName, isChineseLanguageKey } from './utils/responseLanguage';
...
const ZHIPU_MODEL = "glm-4.7-flashx"
const ZHIPU_BASE_URL = "https://open.bigmodel.cn/api/paas/v4"
const ZHIPU_MAX_OUTPUT_TOKENS = 8192
...
// On the live answer path a healthy connect is sub-second, and a stalled connect
// should fail over fast. 4s leaves headroom for a transient DNS hiccup (the
// in-fetch DNS retry adds ~1s) without a long tail. The TTFT race
// (textStreamFallback) handles the separate case of a fast connect that then
// prefills slowly. Override per-call for non-interactive use.
...
// thinkingConfigForModel() in the upstream gateway’s flashModelPicker.
...
export class LLMHelper {
  ...
  // Zhipu GLM is OpenAI-compatible; keep a provider-specific client/key for routing.
  private zhipuClient: OpenAI | null = null
  ...
  constructor(apiKey?: string, useOllama: boolean = false, ollamaModel?: string, ollamaUrl?: string, groqApiKey?: string, openaiApiKey?: string, claudeApiKey?: string, deepseekApiKey?: string, zhipuApiKey?: string) {
    ...
    if (zhipuApiKey) {
      ...
      this.zhipuClient = new OpenAI({ apiKey: zhipuApiKey, baseURL: ZHIPU_BASE_URL })
      console.log(`[LLMHelper] Zhipu client initialized with model: ${ZHIPU_MODEL}`)
    }
  ...
  public setZhipuApiKey(apiKey: string) {
    const trimmed = (apiKey || '').trim();
    if (!trimmed) {
      this.zhipuApiKey = null;
      this.zhipuClient = null;
      console.log("[LLMHelper] Zhipu API Key cleared.");
      return;
    }
    this.zhipuApiKey = trimmed;
    this.zhipuClient = new OpenAI({ apiKey: trimmed, baseURL: ZHIPU_BASE_URL });
    this.textHealth.delete('zhipu');
    console.log("[LLMHelper] Zhipu API Key updated.");
  }
  ...
  public async initModelVersionManager(): Promise<void> {
    ...
    try {
      (global as any).__miaodaGetLLMHelper = () => this;
    } catch {
  ...
  private isZhipuModel(modelId: string): boolean {
    if (!modelId) return false;
  ...
  private getZhipuMaxOutput(_modelId: string): number {
    return ZHIPU_MAX_OUTPUT_TOKENS;
  }
  ...
  public setModel(modelId: string, customProviders: (CustomProvider | CurlProvider)[] = []) {
    ...
    if (modelId === 'zhipu' || modelId === 'glm') targetModelId = ZHIPU_MODEL;
...
ANSWER DIRECTLY:`;
  ...
  private buildLanguageInstructionSuffix(): string {
    if (!this.aiResponseLanguage || this.aiResponseLanguage === 'auto') {
      if (isChineseLanguageKey(this.sttLanguage)) {
        return `\n\n[LANGUAGE INSTRUCTION - HIGHEST PRIORITY]
The current speech-recognition language is Chinese.
Unless the user's latest message explicitly asks for another language, ALWAYS respond in Simplified Chinese.
Keep code, API names, database terms, and proper nouns in their original English when that is natural, but explain in Chinese.
Do NOT answer in English just because the internal system prompt or button label is English.
[END LANGUAGE INSTRUCTION]`;
      }
      const fallbackLanguage = autoLanguageFallbackName(this.sttLanguage);
      return `\n\n[LANGUAGE INSTRUCTION — HIGHEST PRIORITY]
...
If the language is ambiguous, default to ${fallbackLanguage}.
...
[REMINDER] Your entire response MUST be in ${lang} only. Never switch to English.`;
   ...
   *   - custom/curl: server-controlled; we skip (no client-side cache).
   *
  ...
  public async prewarmPromptCache(): Promise<void> {
    try {
      ...
      if (!this.useOllama && this.isClaudeModel(this.currentModelId) && this.claudeClient) {
        ...
        // custom / curl — server-side caching, nothing to prime client-side.
        return;
      }
  ...
  public async chatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, alternateGroqMessage?: string): Promise<string> {
    try {
      ...
      if (this.isZhipuModel(this.currentModelId) && this.zhipuClient) {
        // Zhipu GLM is text-only in this integration; image requests fall through
        // to the vision-capable fallback chain.
        if (!cloudIsMultimodal) {
          return await this.generateWithZhipu(cloudUserContent, openaiSystemPrompt);
        }
      }
      ...
      const routedProviders = routeWithScopeFallback({
        ...
        availability: {
          ...
          hasZhipu: Boolean(this.zhipuClient),
        ...
        models: {
          ...
          zhipu: this.isZhipuModel(this.currentModelId) ? this.currentModelId : ZHIPU_MODEL,
      ...
      for (const routedProvider of routedProviders) {
        ...
        switch (routedProvider.provider) {
          ...
          case 'zhipu':
            if (!cloudIsMultimodal) {
              providers.push({ name: routedProvider.name, execute: () => this.generateWithZhipu(cloudUserContent, openaiSystemPrompt, routedProvider.model || ZHIPU_MODEL) });
            }
            break;
          case 'ollama':
      ...
      if (providers.length === 0) {
        if (cloudIsMultimodal && (this.deepseekClient || this.zhipuClient)) {
          return "The configured text-only provider cannot analyze images. Add a vision-capable provider like Gemini, OpenAI, Claude, or Groq to analyze screenshots.";
        }
  ...
  /**
   * Non-streaming Zhipu GLM generation via the OpenAI-compatible API.
   * Text-only: screenshot/image requests are routed to vision-capable providers.
   */
  private async generateWithZhipu(userMessage: string, systemPrompt?: string, modelId?: string): Promise<string> {
    if (this.isLocalOnlyMode) throw new Error("Cloud providers disabled in local-only mode");
    if (!this.zhipuClient) throw new Error("Zhipu client not initialized");
    this.assertOutboundScopes('zhipu', userMessage);

    await this.rateLimiters.zhipu.acquire();

    const model = modelId || (this.isZhipuModel(this.currentModelId) ? this.currentModelId : ZHIPU_MODEL);
    const messages: any[] = [];
    if (systemPrompt) messages.push({ role: "system", content: systemPrompt });
    messages.push({ role: "user", content: userMessage });

    const response = await this.withTimeout(
      this.withRetry(() => this.zhipuClient!.chat.completions.create({
        model,
        messages,
        temperature: INTERACTIVE_TEMPERATURE,
        max_tokens: this.getZhipuMaxOutput(model),
      })),
      60000,
      `Zhipu (${model})`
    );
  ...
  public async * streamChatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    ...
    if (isMultimodal) {
      ...
      // TEXT-ONLY PROVIDER ORDER: Groq -> Codex CLI -> OpenAI -> Claude -> Gemini Flash-Lite -> Gemini Flash -> Gemini Pro
      if (this.groqClient) {
        // CACHE: pass system separately so Groq prefix-cache hits across turns.
      ...
      if (this.zhipuClient) {
        const glmModel = this.isZhipuModel(this.currentModelId) ? this.currentModelId : ZHIPU_MODEL;
        providers.push({ name: `Zhipu GLM (${glmModel})`, execute: () => this.streamWithZhipu(userContent, openaiSystemPrompt, glmModel, abortSignal) });
      }
    ...
    if (providers.length === 0) {
      if (isMultimodal && imagePaths && (this.deepseekClient || this.zhipuClient)) {
        yield "The configured text-only provider cannot analyze images. Add a vision-capable provider like Gemini, OpenAI, Claude, or Groq to analyze screenshots.";
        return;
      }
        ...
        : this.isOpenAiModel(this.currentModelId) ? 'OpenAI'
          : this.isGroqModel(this.currentModelId) ? 'Groq'
            : this.isDeepseekModel(this.currentModelId) ? 'DeepSeek'
              : this.isZhipuModel(this.currentModelId) ? 'Zhipu GLM'
  ...
  // when the selected model timed out and only Gemini remained.
  //
  ...
  private async * _streamChatInner(
    ...
    // pricing questions answered as "I'm an AI assistant. I don't have
    ...
    // which would dead-end when the selected model failed and
    ...
    // GROQ FAST TEXT OVERRIDE (Text-Only) — call the local Groq key directly.
    //
    ...
    if (fastModeApplies) {
      ...
      if (this.groqClient && !this._groqLocalDisabled) {
        ...
        try {
          ...
          // we'd send a Gemini ID as the Groq model name → 400.
        ...
        // Local Groq failed — fall through to the normal routing below.
      }
    }
    ...
    // vision-first chain (Gemini/Claude/OpenAI) handles them instead.
    ...
    if (this.isZhipuModel(this.currentModelId) && this.zhipuClient && !(isMultimodal && imagePaths)) {
      const zhipuSystem = systemPromptOverride || OPENAI_SYSTEM_PROMPT;
      const finalZhipuSystem = this.injectLanguageInstruction(zhipuSystem);
      yield* this.streamWithZhipu(userContent, finalZhipuSystem, undefined, abortSignal);
      return;
    }
    ...
    if (this.client) {
      ...
      try {
        ...
        console.warn('[LLMHelper] Gemini cascade failed pre-commit:', e?.message || e);
      }
    }
  ...
  /**
   * Stream response from Zhipu GLM (OpenAI-compatible). Text-only by design.
   */
  private async * streamWithZhipu(userMessage: string, systemPrompt?: string, modelId?: string, abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    if (this.isLocalOnlyMode) throw new Error("Cloud providers disabled in local-only mode");
    if (!this.zhipuClient) throw new Error("Zhipu client not initialized");
    this.assertOutboundScopes('zhipu', userMessage);
    ...
    if (abortSignal?.aborted) return;
    const stream = await this.zhipuClient.chat.completions.create({
      model,
      messages,
      stream: true,
      temperature: INTERACTIVE_TEMPERATURE,
  ...
  public getZhipuClient(): OpenAI | null {
    return this.zhipuClient;
  }

  public hasZhipu(): boolean {
  ...
  public async generateMeetingSummary(systemPrompt: string, context: string, groqSystemPrompt?: string): Promise<string> {
    ...
    // summarise, the provider fallback chain (Codex → Groq → Gemini
