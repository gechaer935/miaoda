 ...
 * MIAODA_INTELLIGENCE_ATTRIBUTION=true env (so it can be turned on without enabling
...
export function recordAttribution(input: AttributionInput): IntelligenceAttribution {
  ...
  try {
    let on = false;
    try {
      const env = (process.env.MIAODA_INTELLIGENCE_ATTRIBUTION || '').trim().toLowerCase();
      const traceEnv = (process.env.MIAODA_INTELLIGENCE_TRACE || '').trim().toLowerCase();
