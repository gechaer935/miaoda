...
export function assemblePromptV2(input: AssemblePromptV2Input): AssembledPromptV2 {
  ...
  try {
    ...
    if (v.assistantIdentityWouldLeak) {
      perspectiveGuard = 'You are answering AS the candidate/user in first person. Never say "I am 秒答", "I am an AI assistant", or otherwise self-identify as the assistant — the user expects their own identity. (Genuine questions about the app itself are exempt.)';
    }
  } catch { /* keep empty */ }
