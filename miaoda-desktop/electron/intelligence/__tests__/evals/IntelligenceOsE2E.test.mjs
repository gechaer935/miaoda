...
describe('E2E — Profile category', () => {
  test('Alice: identity + intro + projects + role fit, no 秒答 leak', () => {
    ...
    assert.doesNotMatch(blob, /I'?m 秒答|an AI assistant/i);
  });
});
...
describe('E2E — Fusion + Prompt assembly category', () => {
  test('full pipeline: route → fuse → assemble, with inclusion report', () => {
    ...
    assert.match(out.perspectiveGuard, /Never say "I am 秒答"/);
  });
});
