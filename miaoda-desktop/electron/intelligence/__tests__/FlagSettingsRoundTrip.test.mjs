...
// precedence: env override (MIAODA_*) → SettingsManager.get(<settingKey>) → default(false).
...
//   • the ENV-OVERRIDE resolution chain — set MIAODA_*=1 → enabled true; unset → false.
...
// All MIAODA_* env vars these flags read — cleared before/after so a leaked env from the
...
const ALL_ENV_VARS = [
  'MIAODA_INTELLIGENCE_TRACE',
  'MIAODA_DURABLE_MEMORY_WINDOW',
  'MIAODA_INTELLIGENCE_OS',
  'MIAODA_PROFILE_TREE_V2',
  'MIAODA_CONTEXT_ROUTER_V2',
  'MIAODA_LIVE_TRANSCRIPT_BRAIN',
  'MIAODA_PROMPT_ASSEMBLER_V2',
  'MIAODA_ANSWER_DIVERSITY_GUARD',
  'MIAODA_MEETING_MEMORY_V2',
  'MIAODA_MEETING_SUMMARY_V3',
  'MIAODA_MEETING_MODE_AUTODETECT',
  'MIAODA_FOLLOWUP_DRAFT_V2',
  'MIAODA_SPEAKER_LABELS_V1',
  'MIAODA_MEETING_NOTES_STRUCTURED_OUTPUT',
  'MIAODA_MEETING_SUMMARY_LLM_POLISH',
  'MIAODA_SPEAKER_DIARIZATION_V1',
  'MIAODA_GLOBAL_SEARCH_V2',
  'MIAODA_IN_MEETING_SEARCH_V2',
  'MIAODA_CONVERSATION_MEMORY_V2',
  'MIAODA_LECTURE_INTELLIGENCE_V2',
  'MIAODA_DIAGRAM_INTELLIGENCE',
  'MIAODA_HINDSIGHT_MEMORY',
  'MIAODA_HINDSIGHT_LIVE_RECALL',
  'MIAODA_HINDSIGHT_POST_MEETING_RETAIN',
];
...
describe('Phase 14 — intelligence flag settings contract (key + meta surface)', () => {
  ...
  test('intelligenceFlagMeta(key) returns {setting, env, default:false} for every flag', () => {
    for (const key of intelligenceFlagKeys()) {
      ...
      assert.ok(meta.env.startsWith('MIAODA_'), `${key}.env follows MIAODA_ convention (${meta.env})`);
  ...
  test('intelligenceFlagMeta exact values for several named flags', () => {
    assert.deepEqual(intelligenceFlagMeta('trace'),
      { setting: 'intelligenceTraceEnabled', env: 'MIAODA_INTELLIGENCE_TRACE', default: false });
    assert.deepEqual(intelligenceFlagMeta('durableMemoryWindow'),
      { setting: 'intelligenceDurableMemoryWindow', env: 'MIAODA_DURABLE_MEMORY_WINDOW', default: false });
    assert.deepEqual(intelligenceFlagMeta('conversationMemoryV2'),
      { setting: 'conversationMemoryV2Enabled', env: 'MIAODA_CONVERSATION_MEMORY_V2', default: false });
    assert.deepEqual(intelligenceFlagMeta('lectureIntelligenceV2'),
      { setting: 'lectureIntelligenceV2Enabled', env: 'MIAODA_LECTURE_INTELLIGENCE_V2', default: false });
    assert.deepEqual(intelligenceFlagMeta('diagramIntelligence'),
      { setting: 'diagramIntelligenceEnabled', env: 'MIAODA_DIAGRAM_INTELLIGENCE', default: false });
    assert.deepEqual(intelligenceFlagMeta('hindsightMemory'),
      { setting: 'hindsightMemoryEnabled', env: 'MIAODA_HINDSIGHT_MEMORY', default: false });
  });
...
describe('Phase 14 — ENV override resolution chain (the mechanism the UI/IPC relies on)', () => {
  ...
  test('env override is PER-FLAG (toggling one does not affect another)', () => {
    ...
    delete process.env.MIAODA_LECTURE_INTELLIGENCE_V2;
  });
  ...
  test('intelligenceFlagSnapshot() reflects the resolved state of the env override', () => {
    ...
    process.env.MIAODA_INTELLIGENCE_TRACE = 'on';
    __resetIntelligenceFlagsCache();
