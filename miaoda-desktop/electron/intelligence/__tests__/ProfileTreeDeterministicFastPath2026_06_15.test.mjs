...
const PROFILE = {
  ...
  projects: [{ name: '秒答', description: 'AI meeting copilot', technologies: ['Electron', 'TypeScript'] }],
...
describe('ProfileTree deterministic fast path — first-person, provider-free', () => {
  ...
  test('name → first person, never "Your name"', () => {
    ...
    assert.match(a, /^My name is 秒答\.?$/);
  });

  test('intro → first person, grounded, never "I\'m 秒答"', () => {
    const a = svc.getInterviewIntro();
    assert.match(a, /^I'm 秒答/);
    assert.doesNotMatch(a, /秒答, an AI|I am 秒答/i);
  });
...
describe('App identity vs Candidate identity', () => {
  ...
  const APP = ['are you an AI', 'are you a bot', 'what is 秒答', 'what model are you', 'who built you'];
