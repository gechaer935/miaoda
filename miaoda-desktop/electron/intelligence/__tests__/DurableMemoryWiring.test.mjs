...
// `durableMemoryWindow` flag (env MIAODA_DURABLE_MEMORY_WINDOW).
//
...
function clearEnv() {
  delete process.env.MIAODA_DURABLE_MEMORY_WINDOW;
  __resetIntelligenceFlagsCache();
}
...
describe('durableMemoryWindow flag: flips the source, defaults OFF, fresh read', () => {
  ...
  test('MIAODA_DURABLE_MEMORY_WINDOW=1 enables it (fresh env read, no cache)', () => {
    process.env.MIAODA_DURABLE_MEMORY_WINDOW = '1';
