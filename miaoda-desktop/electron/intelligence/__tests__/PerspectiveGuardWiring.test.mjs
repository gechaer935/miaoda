...
// / MIAODA_PROFILE_TREE_V2, default OFF).
//
...
const MIAODA_LEAK = /\bi'?m natively\b|\bas an ai assistant\b/i;

// The candidate-identity asks that, in a candidate-voice mode, MUST be answered in the
// candidate's voice — never "I'm 秒答". These are intentionally NOT in the
...
describe('Phase 3 safety — widening the trigger never over-strips a clean answer', () => {
  ...
  // misclassified to a non-candidate answerType that tail-leaks "I'm 秒答").
  test('a genuine assistant-meta tail IS stripped while the valid content survives', () => {
    const leaky = "I'm a Senior ML Engineer at Acme AI with five years of experience. I'm 秒答, an AI assistant, so I can't share personal experiences.";
    ...
    assert.doesNotMatch(s.text, MIAODA_LEAK, 'the "I\'m 秒答 / AI assistant" leak is gone');
  });
  ...
  // widened for them). Belt-and-suspenders: even if it did, a plain "I'm 秒答, an AI
