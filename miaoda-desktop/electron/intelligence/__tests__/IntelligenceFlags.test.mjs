...
const ENV_KEYS = [
  'MIAODA_INTELLIGENCE_TRACE', 'MIAODA_DURABLE_MEMORY_WINDOW', 'MIAODA_INTELLIGENCE_OS',
  'MIAODA_PROFILE_TREE_V2', 'MIAODA_CONTEXT_ROUTER_V2', 'MIAODA_LIVE_TRANSCRIPT_BRAIN',
  'MIAODA_PROMPT_ASSEMBLER_V2', 'MIAODA_ANSWER_DIVERSITY_GUARD', 'MIAODA_MEETING_MEMORY_V2',
  'MIAODA_MEETING_SUMMARY_V3', 'MIAODA_MEETING_MODE_AUTODETECT', 'MIAODA_FOLLOWUP_DRAFT_V2',
  'MIAODA_SPEAKER_LABELS_V1', 'MIAODA_MEETING_NOTES_STRUCTURED_OUTPUT',
  'MIAODA_MEETING_SUMMARY_LLM_POLISH', 'MIAODA_SPEAKER_DIARIZATION_V1',
  'MIAODA_GLOBAL_SEARCH_V2', 'MIAODA_IN_MEETING_SEARCH_V2', 'MIAODA_CONVERSATION_MEMORY_V2',
  'MIAODA_LECTURE_INTELLIGENCE_V2', 'MIAODA_DIAGRAM_INTELLIGENCE', 'MIAODA_HINDSIGHT_MEMORY',
  'MIAODA_HINDSIGHT_LIVE_RECALL', 'MIAODA_HINDSIGHT_POST_MEETING_RETAIN',
];
...
describe('intelligenceFlags', () => {
  ...
  test('a newly-added flag can be toggled by env independently', () => {
    process.env.MIAODA_CONTEXT_ROUTER_V2 = 'on';
    __resetIntelligenceFlagsCache();
  ...
  test('env override accepts on/true/enabled/yes', () => {
    for (const v of ['on', 'true', 'enabled', 'yes', '1']) {
      process.env.MIAODA_DURABLE_MEMORY_WINDOW = v;
      __resetIntelligenceFlagsCache();
  ...
  test('env override OFF wins even if default were ON', () => {
    for (const v of ['off', 'false', '0', 'disabled', 'no']) {
      process.env.MIAODA_INTELLIGENCE_TRACE = v;
      __resetIntelligenceFlagsCache();
  ...
  test('unknown env value falls through to default OFF', () => {
    process.env.MIAODA_INTELLIGENCE_TRACE = 'maybe';
    __resetIntelligenceFlagsCache();
    assert.equal(isIntelligenceTraceEnabled(), false);
  });
