...
export class CropperWindowHelper {
    ...
    private applyNativeCaptureProtection(enable: boolean): void {
        ...
        try {
            ...
            if (typeof method === 'function') {
                method(this.cropperWindow.getNativeWindowHandle());
            }
        } catch (e) {
            console.warn(`[CropperWindowHelper] Failed to ${enable ? 'apply' : 'remove'} native capture protection:`, e);
        }
    }
    ...
    public reassertContentProtection(): void {
        if (this.cropperWindow && !this.cropperWindow.isDestroyed()) {
            ...
            this.applyNativeCaptureProtection(this.isUndetectable);
        }
    }
