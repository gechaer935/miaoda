...
interface ElectronAPI {
  ...
  takeScreenshot: (options?: { hideWindows?: boolean }) => Promise<void>;
  ...
  liteAnalyzeWrittenImage: (
    path: string,
    mode?: 'auto' | 'glm-4.6v-flashx' | 'glm-ocr' | 'qwen3.6-flash' | 'qwen3.7-plus',
  ) => Promise<{ text: string; provider?: string; model?: string; source?: string; failures?: string[] }>;
  ...
  testLlmConnection: (
    provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu',
    apiKey?: string,
  ) => Promise<{ success: boolean; error?: string }>;
  ...
  setZhipuApiKey: (apiKey: string) => Promise<{ success: boolean; error?: string }>;
  ...
  miaodaAsrSetModel: (model: 'paraformer-realtime-v2' | 'fun-asr-realtime' | 'qwen3-asr-flash-realtime') => Promise<string>;
  ...
  miaodaWrittenSolveScreen: (input: {
    filePath: string;
  ...
  setZhipuSttApiKey: (apiKey: string) => Promise<{ success: boolean; error?: string }>;
  ...
  onNativeAudioTranscript: (
    callback: (transcript: { speaker: string; text: string; final: boolean; segmentId?: string; speechEnded?: boolean }) => void,
  ) => () => void;
  onNativeAudioSpeechStarted: (
    callback: (event: { speaker: string; timestamp: number }) => void,
  ) => () => void;
  onNativeAudioSpeechEnded: (
    callback: (event: { speaker: string; timestamp: number }) => void,
  ) => () => void;
  ...
  liteKnowledgeInfo: () => Promise<{ count: number; files: string[] }>;
  liteKnowledgePick: () => Promise<{ success: boolean; cancelled?: boolean; file?: string; count?: number; files?: string[]; error?: string }>;
  liteKnowledgeMatch: (question: string, threshold?: number) => Promise<{
    matched: boolean;
    question: string;
    answer: string;
    score: number;
  ...
  liteAnswer: (
    question: string,
    options?: { promptInstruction?: string; model?: string },
  ) => Promise<{ success: boolean; answer: string; model?: string; error?: string }>;
  generateFollowUp: (
    intent: string,
    userRequest?: string,
  ...
  pinWindowTopmost: () => Promise<{ success: boolean }>;
  ...
  onAppUpdateState: (callback: (state: {
  ...
  profileExtractResumeText: (filePath: string) => Promise<{
    success: boolean;
    fileName?: string;
    text?: string;
    chars?: number;
...
// Build the complete legacy bridge for development compatibility, but expose
// only the small allow-list used by the production Miaoda Lite surfaces.
const fullElectronAPI = {
  ...
  takeScreenshot: (options?: { hideWindows?: boolean }) => ipcRenderer.invoke('take-screenshot', options),
  ...
  openAssistantWindow: (kind: 'interview' | 'written') => ipcRenderer.invoke('assistant-window:open', kind),
  closeAssistantWindow: () => ipcRenderer.invoke('assistant-window:close'),
  hideAssistantWindow: () => ipcRenderer.invoke('assistant-window:hide'),
  resizeAssistantWindow: (height: number) => ipcRenderer.invoke('assistant-window:resize', height),
  ...
  liteAnalyzeWrittenImage: (
    path: string,
    mode?: 'auto' | 'glm-4.6v-flashx' | 'glm-ocr' | 'qwen3.6-flash' | 'qwen3.7-plus',
  ) => ipcRenderer.invoke('lite:analyze-written-image', path, mode),
  ...
  pinWindowTopmost: () => ipcRenderer.invoke('window-pin-topmost'),
  ...
  testLlmConnection: (provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu', apiKey: string) =>
  ...
  setZhipuApiKey: (apiKey: string) => ipcRenderer.invoke('set-zhipu-api-key', apiKey),
  ...
  miaodaAuthGetState: () => ipcRenderer.invoke('miaoda-auth:get-state'),
  miaodaAuthSetBaseUrl: (url: string) => ipcRenderer.invoke('miaoda-auth:set-base-url', url),
  miaodaAsrGetModel: () => ipcRenderer.invoke('miaoda-asr:get-model'),
  miaodaAsrSetModel: (model: 'paraformer-realtime-v2' | 'fun-asr-realtime' | 'qwen3-asr-flash-realtime') => ipcRenderer.invoke('miaoda-asr:set-model', model),
  miaodaAsrGetPreRollEnabled: () => ipcRenderer.invoke('miaoda-asr:get-preroll-enabled'),
  miaodaAsrSetPreRollEnabled: (enabled: boolean) => ipcRenderer.invoke('miaoda-asr:set-preroll-enabled', enabled),
  miaodaAccountLogin: (username: string, password: string) => ipcRenderer.invoke('miaoda-auth:account-login', username, password),
  miaodaAccountRegister: (username: string, password: string) => ipcRenderer.invoke('miaoda-auth:account-register', username, password),
  miaodaAccountRedeem: (cardKey: string) => ipcRenderer.invoke('miaoda-account:redeem', cardKey),
  miaodaAuthLogout: () => ipcRenderer.invoke('miaoda-auth:logout'),
  miaodaAuthGetQuota: () => ipcRenderer.invoke('miaoda-auth:get-quota'),
  miaodaAuthHeartbeat: (interviewActive?: boolean) => ipcRenderer.invoke('miaoda-auth:heartbeat', interviewActive),
  onMiaodaQuotaChanged: (callback: (quota: any) => void) => {
    const listener = (_event: unknown, quota: any) => callback(quota);
    ipcRenderer.on('miaoda-auth:quota-changed', listener);
    return () => ipcRenderer.removeListener('miaoda-auth:quota-changed', listener);
  },
  miaodaCompanionCreate: () => ipcRenderer.invoke('miaoda-companion:create'),
  miaodaCompanionResume: () => ipcRenderer.invoke('miaoda-companion:resume'),
  miaodaCompanionSend: (type: string, payload: Record<string, unknown> = {}, requestId?: string) => ipcRenderer.invoke('miaoda-companion:send', type, payload, requestId),
  miaodaCompanionUploadCapture: (input: { captureId: string; filePath: string; type?: string; language?: string; visionMode?: string }) => ipcRenderer.invoke('miaoda-companion:upload-capture', input),
  miaodaCompanionDisconnect: () => ipcRenderer.invoke('miaoda-companion:disconnect'),
  onMiaodaCompanionEvent: (callback: (event: any) => void) => {
    const listener = (_event: unknown, payload: any) => callback(payload);
    ipcRenderer.on('miaoda-companion:event', listener);
    return () => ipcRenderer.removeListener('miaoda-companion:event', listener);
  },
  miaodaInterviewAnswer: (input: {
    question: string;
    job?: string;
    ...
    pipelineId?: string;
    attemptId?: number;
  }) => ipcRenderer.invoke('miaoda-interview:answer', input),
  miaodaInterviewAnswerStream: (input: {
    question: string;
    job?: string;
    language?: string;
    model?: string;
    ...
    pipelineId?: string;
    attemptId?: number;
  }, onToken: (token: string) => void, onAccepted?: (info: { receivedAtUnixMs?: number }) => void) => new Promise((resolve, reject) => {
    const streamId = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const channel = 'miaoda-interview:answer-stream-event';
    const listener = (_event: unknown, payload: any) => {
      if (!payload || payload.streamId !== streamId) return;
      if (payload.type === 'accepted') {
        onAccepted?.({ receivedAtUnixMs: payload.receivedAtUnixMs });
        return;
      }
      ...
      if (payload.type === 'done') {
        resolve({ answer: String(payload.answer || ''), model: payload.model });
      } else {
        reject(new Error(String(payload.message || 'stream failed')));
      }
    };
    const watchdog = setTimeout(() => {
      ipcRenderer.removeListener(channel, listener);
      reject(new Error('Interview answer stream timed out'));
    }, 100_000);
    ipcRenderer.on(channel, listener);
    ipcRenderer.send('miaoda-interview:answer-stream-start', { ...input, streamId });
  }),
  miaodaInterviewTranslateStream: (input: {
    text: string;
    sourceLanguage?: string;
  }, onToken: (token: string) => void, onAccepted?: (info: { receivedAtUnixMs?: number }) => void) => new Promise((resolve, reject) => {
    const streamId = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const channel = 'miaoda-interview:translate-stream-event';
    const listener = (_event: unknown, payload: any) => {
      if (!payload || payload.streamId !== streamId) return;
      if (payload.type === 'accepted') {
        onAccepted?.({ receivedAtUnixMs: payload.receivedAtUnixMs });
      ...
      ipcRenderer.removeListener(channel, listener);
      if (payload.type === 'done') {
        resolve({ translation: String(payload.translation || ''), model: payload.model });
      } else {
        reject(new Error(String(payload.message || 'translation failed')));
      }
    };
    ipcRenderer.on(channel, listener);
    ipcRenderer.send('miaoda-interview:translate-stream-start', { ...input, streamId });
  }),
  miaodaWrittenSolveScreen: (input: {
    filePath: string;
    type?: 'auto' | 'code' | 'leetcode' | 'assessment';
    language?: string;
    visionMode?: 'auto' | 'aliyun:qwen3.7-plus' | 'aliyun:qwen3.6-plus';
  }) => ipcRenderer.invoke('miaoda-written:solve-screen', input),
  ...
  setZhipuSttApiKey: (apiKey: string) => ipcRenderer.invoke('set-zhipu-stt-api-key', apiKey),
  ...
  onNativeAudioSpeechStarted: (
    ...
    const subscription = (_: any, data: any) => callback(data);
    ipcRenderer.on('native-audio-speech-started', subscription);
    return () => {
      ipcRenderer.removeListener('native-audio-speech-started', subscription);
    };
  },
  onNativeAudioSpeechEnded: (
    callback: (event: { speaker: string; timestamp: number }) => void,
  ...
  onMeetingAudioError: (callback: (message: string) => void) => {
    const subscription = (_: any, message: string) => callback(String(message || 'Audio pipeline failed'));
  ...
  liteKnowledgeInfo: () => ipcRenderer.invoke('lite:knowledge-info'),
  liteKnowledgePick: () => ipcRenderer.invoke('lite:knowledge-pick'),
  liteKnowledgeMatch: (question: string, threshold?: number) =>
    ipcRenderer.invoke('lite:knowledge-match', question, threshold),
  liteAnswer: (question: string, options?: { promptInstruction?: string; model?: string }) =>
    ipcRenderer.invoke('lite:answer', question, options),
  ...
  onAppUpdateState: (callback: (state: {
    ...
    return () => {
      ipcRenderer.removeListener('app-update-state', subscription);
    };
  },
  getAppUpdateState: () => ipcRenderer.invoke('get-update-state'),
  ...
  setKeybindRecordingSuspended: (suspended: boolean) =>
    ipcRenderer.invoke('keybinds:set-recording-suspended', suspended),
  onKeybindRecordingCancelled: (callback: () => void) => {
    const subscription = () => callback();
    ipcRenderer.on('keybinds:recording-cancelled', subscription);
    return () => ipcRenderer.removeListener('keybinds:recording-cancelled', subscription);
  },
  onKeybindRecordingInput: (callback: (input: { key: string; ctrlKey: boolean; altKey: boolean; shiftKey: boolean; metaKey: boolean }) => void) => {
    const subscription = (_: any, input: { key: string; ctrlKey: boolean; altKey: boolean; shiftKey: boolean; metaKey: boolean }) => callback(input);
    ipcRenderer.on('keybinds:recording-input', subscription);
    return () => ipcRenderer.removeListener('keybinds:recording-input', subscription);
  },
  ...
  profileExtractResumeText: (filePath: string) => ipcRenderer.invoke('profile:extract-resume-text', filePath),
  ...
  fetchProviderModels: (provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu', apiKey: string) =>
    ipcRenderer.invoke('fetch-provider-models', provider, apiKey),
  setProviderPreferredModel: (provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu', modelId: string) =>
...
} as ElectronAPI & Record<string, any>;

const liteApiKeys = [
  'closeAssistantWindow', 'cropperCancelled', 'cropperConfirmed', 'endMeeting',
  'getAppUpdateState', 'getKeybinds', 'getOverlayMousePassthrough', 'getThemeMode', 'getUndetectable',
  'miaodaAccountLogin', 'miaodaAccountRegister', 'miaodaAccountRedeem', 'miaodaAuthGetQuota', 'miaodaAuthGetState',
  'miaodaCompanionCreate', 'miaodaCompanionResume', 'miaodaCompanionSend',
  'miaodaCompanionUploadCapture', 'miaodaInterviewAnswer', 'miaodaInterviewAnswerStream', 'miaodaInterviewTranslateStream',
  'miaodaWrittenSolveScreen', 'onGlobalShortcut', 'onKeybindRecordingCancelled',
  'onKeybindRecordingInput', 'onKeybindsUpdate', 'onMeetingAudioError',
  'onMiaodaCompanionEvent', 'onMiaodaQuotaChanged', 'onNativeAudioSpeechEnded', 'onNativeAudioSpeechStarted',
  'onAppUpdateState', 'onNativeAudioTranscript', 'onOverlayMousePassthroughChanged', 'onResetCropper',
  'onSttStatusChanged', 'onThemeChanged', 'onUndetectableChanged', 'openAssistantWindow',
  'downloadUpdate', 'openExternal', 'platform', 'quitApp', 'resetKeybinds', 'resizeAssistantWindow', 'restartAndInstall',
  'setKeybind', 'setKeybindRecordingSuspended', 'setOverlayMousePassthrough', 'setRecognitionLanguage',
  'setUndetectable', 'startMeeting', 'takeScreenshot', 'toggleSettingsWindow',
  'updateContentDimensions', 'writeClipboardText',
] as const;

// Never allow the broad legacy bridge in a packaged build. Environment
// variables are controlled by the local process owner and therefore are not a
// security boundary for a distributed desktop client.
const exposeLegacyBridge = process.env.NODE_ENV === 'development'
  && process.env.MIAODA_LEGACY_FEATURES === '1';

const exposedElectronAPI = exposeLegacyBridge
  ? fullElectronAPI
  : Object.fromEntries(liteApiKeys.map((key) => [key, fullElectronAPI[key]]));

contextBridge.exposeInMainWorld('electronAPI', exposedElectronAPI);
...
// output appears in ~/Documents/miaoda_debug.log alongside main-process logs.
