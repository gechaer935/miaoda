...
import { buildTechnicalInterviewFallbackAnswer, isTechnicalInterviewQuestion, shouldUseTechnicalInterviewFallback } from './llm/technicalInterviewFallback';
...
import { LiteInterviewKnowledgeBase } from './services/LiteInterviewKnowledgeBase';
...
export class IntelligenceEngine extends EventEmitter {
    ...
    private lastAutoAnswerQuestion: string = '';
    private lastAutoAnswerAt: number = 0;
    ...
    private static hasQuestionSignal(text: string): boolean {
        ...
        if (isTechnicalInterviewQuestion(trimmed)) return true;
        return /\b(what|how|why|where|when|which|who|can you|could you|tell me|explain|describe|walk me through|talk me through)\b/i.test(trimmed)
            || /(什么是|什么叫|为什么|怎么|如何|区别|有哪些|讲一下|说一下|解释一下|介绍一下)/.test(trimmed);
    }

    private maybeAutoAnswerFinalQuestion(segment: TranscriptSegment): void {
        if (segment.speaker !== 'interviewer' || !segment.final) return;
        if (this.activeMode !== 'idle' && this.activeMode !== 'assist' && this.activeMode !== 'what_to_say') return;

        const text = (segment.text || '').trim();
        if (text.length < 6 || !IntelligenceEngine.hasQuestionSignal(text)) return;

        const now = Date.now();
        const similarity = this.lastAutoAnswerQuestion
            ? IntelligenceEngine.jaccardSimilarity(this.lastAutoAnswerQuestion, text)
            : 0;
        if (similarity >= 0.82 && now - this.lastAutoAnswerAt < 30000) return;

        this.lastAutoAnswerQuestion = text;
        this.lastAutoAnswerAt = now;

        const confidence = Math.max(segment.confidence ?? 0.85, 0.85);
        const context = this.session.getFormattedContextWithInterim(180);
        console.log('[IntelligenceEngine] Auto-answer final interviewer question', { length: text.length, confidence });
        void this.handleSuggestionTrigger({
            context,
            lastQuestion: text,
            confidence,
        }).catch(err => console.error('[IntelligenceEngine] Auto-answer trigger failed:', err));
    }
    ...
    handleTranscript(segment: TranscriptSegment, skipRefinementCheck: boolean = false): void {
        ...
        this.maybeAutoAnswerFinalQuestion(segment);
    ...
    async runWhatShouldISay(question?: string, confidence: number = 0.8, imagePaths?: string[], options?: { speculative?: boolean; skipCooldown?: boolean; screenContext?: ScreenContext; promptInstruction?: string; activeSkill?: { id: string; name: string; promptBlock: string }; domContext?: string }): Promise<string | null> {
        ...
        try {
            ...
            // empty candidateProfile lets the model answer "I'm 秒答, an AI
            ...
            const kbQuestion = question || extractedQuestion.latestQuestion || lastInterviewerTurn;
            const kbMatch = LiteInterviewKnowledgeBase.getInstance().match(kbQuestion);
            if (kbMatch && !isSpeculative) {
                const kbAnswer = kbMatch.answer.trim();
                if (kbAnswer.length >= 5) {
                    trace.mark('fallback_answer_used', {
                        reason: 'lite_interview_knowledge_base',
                        score: Number(kbMatch.score.toFixed(3)),
                        elapsedMs: kbMatch.elapsedMs,
                    });
                    this.emit('suggested_answer_token', kbAnswer, kbQuestion || 'What to Answer', confidence, generationId);
                    this.session.addAssistantMessage(kbAnswer);
                    this.session.pushUsage({
                        type: 'assist',
                        timestamp: Date.now(),
                        question: kbQuestion || 'What to Answer',
                        answer: kbAnswer,
                    });
                    this.emit('suggested_answer', kbAnswer, kbQuestion || 'What to Answer', confidence);
                    this.setMode('idle');
                    return kbAnswer;
                }
            }
            ...
            if (liveDeadlineFired && !emittedStreamingToken && !isSpeculative
                ...
                if (hasLiveFallback) {
                    ...
                    const technicalFallback = shouldUseTechnicalInterviewFallback(question || extractedQuestion.latestQuestion || lastInterviewerTurn, answerPlan.answerType)
                        ? buildTechnicalInterviewFallbackAnswer(question || extractedQuestion.latestQuestion || lastInterviewerTurn)
                        : null;
                    const safe = technicalFallback || ((answerPlan.answerType === 'general_meeting_answer' || answerPlan.answerType === 'lecture_answer')
                        ? localizedNoContextFromConversation(this.llmHelper.getAiResponseLanguage(), this.llmHelper.getSttLanguage())
                        : localizedComeBack(this.llmHelper.getAiResponseLanguage(), this.llmHelper.getSttLanguage()));
                    fullAnswer = safe;
                    emitChunk(safe);
                    trace.mark('fallback_answer_used', { answerType: answerPlan.answerType });
                }
            ...
            // "秒答 / an AI assistant", and must NEVER falsely refuse ("I can't
            ...
            // strip an assistant-meta tail ("as an AI assistant", "I'm 秒答", "I
            ...
            if (answerPlan.answerType === 'project_about_answer' || answerPlan.answerType === 'project_answer') {
                if (/^\s*(?:I(?:'m| am) 秒答[.,]?\s*(?:an? AI assistant[.,]?\s*)?)?I\s+(?:cannot|can\s?not|can'?t)\s+share\s+that(?:\s+information)?\s*\.?\s*$/i.test(fullAnswer.trim())) {
            ...
            if (shouldUseTechnicalInterviewFallback(question || extractedQuestion.latestQuestion || lastInterviewerTurn, answerPlan.answerType)
                && /(没有足够|上下文|context|come back|not enough|don't have enough|这个问题可以按|先给定义|展开时建议|如果面试官|建议这样|how to answer)/i.test(fullAnswer)) {
                const technicalFallback = buildTechnicalInterviewFallbackAnswer(question || extractedQuestion.latestQuestion || lastInterviewerTurn);
                if (technicalFallback) {
                    fullAnswer = technicalFallback;
                    trace.mark('repair_used', { reason: 'technical_interview_fallback' });
                }
            }
            ...
            // it") and emit "I'm 秒答, an AI assistant" / "I can't share that"
                ...
                try {
                    ...
                    if (mis.isMisfire) {
                        const technicalFallback = shouldUseTechnicalInterviewFallback(question || extractedQuestion.latestQuestion || lastInterviewerTurn, answerPlan.answerType)
                            ? buildTechnicalInterviewFallbackAnswer(question || extractedQuestion.latestQuestion || lastInterviewerTurn)
                            : null;
                        fullAnswer = technicalFallback || ((answerPlan.answerType === 'general_meeting_answer' || answerPlan.answerType === 'lecture_answer')
                            ? localizedNoContextFromConversation(this.llmHelper.getAiResponseLanguage(), this.llmHelper.getSttLanguage())
                            : answerPlan.answerType === 'sales_answer'
                                ? localizedSalesNeedMore(this.llmHelper.getAiResponseLanguage(), this.llmHelper.getSttLanguage())
                                : localizedNeedMore(this.llmHelper.getAiResponseLanguage(), this.llmHelper.getSttLanguage()));
