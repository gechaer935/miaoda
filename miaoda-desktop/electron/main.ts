...
// The client uses the normal Windows Direct3D/ANGLE renderer and has no
// WebGPU/Vulkan feature. Keeping Vulkan disabled lets the release omit
// Chromium's large software-Vulkan fallback safely.
...
function isApprovedExternalURL(raw: string): boolean {
  try {
    ...
    return target.protocol === 'https:' && APPROVED_EXTERNAL_HOSTS.has(target.hostname.toLowerCase());
  } catch {
    return false;
  }
}

function isDevelopmentRendererURL(raw: string): boolean {
  if (app.isPackaged) return false;
  try {
    const target = new URL(raw);
    return target.protocol === 'http:' && (target.hostname === 'localhost' || target.hostname === '127.0.0.1') && target.port === '5180';
  } catch {
    return false;
  }
}

// Apply the navigation boundary to every renderer, including small utility
// windows created outside WindowHelper. A compromised renderer must not be able
// to turn a privileged preload into an arbitrary remote webpage.
app.on('web-contents-created', (_event, contents) => {
  ...
  contents.on('will-navigate', (event, url) => {
    if (url === contents.getURL() || isDevelopmentRendererURL(url)) return;
 ...
 * `miaodaSigned: true` into the packaged app's package.json via
...
function isSignedBuild(): boolean {
  ...
  try {
    ...
    _cachedSignedBuild = pkg?.miaodaSigned === true
  } catch {
...
const UPDATE_FEED_URL = 'https://example.invalid/download/updates/'
const UPDATE_CHECK_DELAY_MS = 5_000
const UPDATE_CHECK_INTERVAL_MS = 6 * 60 * 60 * 1_000

type AppUpdateStatus = 'idle' | 'checking' | 'available' | 'downloading' | 'downloaded'
...
const getLogFile = (): string | null => {
  ...
  try {
    _logFile = path.join(app.getPath('documents'), 'miaoda_debug.log');
    return _logFile;
  } catch {
 ...
 * `MIAODA_DEV_BYPASS_SCREEN_TCC=1` to restore the legacy bypass for
 * smooth daily development.
 */
function isDevTccBypassEnabled(): boolean {
  return !app.isPackaged && process.env.MIAODA_DEV_BYPASS_SCREEN_TCC === '1';
}
...
function getMacScreenCaptureStatus(): MacScreenCaptureStatus {
  ...
  if (isDevTccBypassEnabled()) {
    console.log('[Main] Dev TCC bypass enabled (MIAODA_DEV_BYPASS_SCREEN_TCC=1) — reporting screen capture as granted');
    return 'granted';
  }
...
function formatPermissionMessage(reason: PermissionReason, extra?: { device?: string }): string {
  ...
  switch (reason) {
    ...
    case 'mac-screen-recording-restricted':
      ...
      return 'Screen Recording is restricted by device policy. Interviewer audio will not be captured. Contact your administrator to allow screen capture for 秒答.';
    case 'mac-screen-recording-revoked-rebuild':
      ...
      if (!isMac) return formatPermissionMessage('system-audio-stuck');
      return 'System audio is being captured but every sample is silent. This usually means macOS Screen Recording permission needs to be re-granted to this build of 秒答. Open System Settings → Privacy & Security → Screen Recording, toggle 秒答 off and back on, then restart the app. (If you recently rebuilt or updated, the previous grant may not apply.)';
    case 'mic-denied':
      return isMac
        ? 'Microphone access denied. Please allow microphone access in System Settings → Privacy & Security → Microphone, then restart 秒答.'
        : 'Microphone access denied. Please allow microphone access in Settings → Privacy → Microphone, then restart 秒答.';
    case 'mic-zero-fill':
      return isMac
        ? 'Microphone is producing silent audio. Check that the device is unmuted and that macOS Microphone permission is granted to 秒答 in System Settings → Privacy & Security → Microphone.'
        : 'Microphone is producing silent audio. Check that the device is unmuted and that 秒答 has microphone access in Settings → Privacy → Microphone.';
...
import { MiaodaStreamingSTT } from "./audio/MiaodaStreamingSTT"
...
// 秒答发行版默认只保留核心面试、笔试和手机联动能力。设置
// MIAODA_LEGACY_FEATURES=1 才启动旧 秒答 的预热与后台服务。
const MIAODA_LIGHTWEIGHT = app.isPackaged || process.env.MIAODA_LEGACY_FEATURES !== '1';
import { DatabaseManager } from "./db/DatabaseManager"

/** Unified type for all STT providers with optional extended capabilities */
type STTProvider = (GoogleSTT | RestSTT | MiaodaStreamingSTT | DeepgramStreamingSTT | SonioxStreamingSTT | ElevenLabsStreamingSTT | OpenAIStreamingSTT) & {
  finalize?: () => void;
...
interface ScreenshotCaptureSession {
  ...
  assistantKind: 'interview' | 'written' | null;
...
// Legacy upstream profile-intelligence ("knowledge") modules. They were loaded at
// runtime from a package that is not part of this distribution, so these have
// always stayed unset and every block guarded by them below is inert. 秒答 routes
// retrieval through RAGManager and the Go backend instead. Left declared (rather
// than deleting the dependent blocks) so the guarded code keeps type-checking.
let KnowledgeOrchestratorClass: any;
let KnowledgeDatabaseManagerClass: any;
// Phase 1: shared comp-evidence detector for transcript-aware intent routing.
let textHasCompEvidence: any;
...
export class AppState {
  ...
  private updateDownloadState: AppUpdateStatus = 'idle'
  ...
  private updateLastError: string | null = null
  ...
  private currentMeetingIsLite: boolean = false;
  ...
  // launcher's "Meeting ongoing" pill switches back to "Start 秒答" the
  ...
  // Per-speaker/per-segment throttle for the display-only
  // `native-audio-transcript` IPC.
  ...
  private _transcriptPartialThrottle = new Map<string, {
    ...
    pending: { speaker: string; text: string; timestamp: number; final: boolean; confidence: number; segmentId?: string; speechEnded?: boolean } | null;
  }>();
  ...
  constructor() {
    ...
    this.isUndetectable = LITE_MODE ? true : (settingsManager.get('isUndetectable') ?? false);
    ...
    if (!MIAODA_LIGHTWEIGHT && (process.platform === 'win32' || process.platform === 'darwin')) {
    ...
    if (!MIAODA_LIGHTWEIGHT) setImmediate(() => {
      try {
    ...
    keybindManager.onShortcutRecordingSuspended((suspended) => {
      this.settingsWindowHelper.setShortcutRecordingActive(suspended);
    });
    this.settingsWindowHelper.setVisibilityChangeHandler((isVisible) => {
      if (!isVisible) keybindManager.setShortcutRecordingSuspended(false);
    });
    ...
    const hasWrittenQuota = async (): Promise<boolean> => {
      try {
        const { MiaodaAuthClient } = require('./services/MiaodaAuthClient');
        const client = MiaodaAuthClient.getInstance();
        const quota = await client.getQuota().catch(() => client.getState().quota);
        const remaining = Number(quota?.remainingWrittenQuestions);
        return !Number.isFinite(remaining) || remaining > 0;
      } catch {
        // Let the renderer/backend perform the final check when quota state is
        // temporarily unavailable instead of breaking a valid shortcut.
        return true;
      }
    };
    ...
    keybindManager.onShortcutTriggered(async (actionId) => {
      ...
      try {
        if (actionId === 'general:quit-app') {
          ...
          // The compact written assistant owns its own solve flow. Route the
          // shortcut there so programming questions use the ACM full-program
          // protocol; non-code questions still follow their detected type.
          // The legacy workspace keeps
          // using the existing capture-and-process pipeline.
          const assistantKind = this.windowHelper.getActiveAssistantKind();
          const assistantWindow = this.windowHelper.getActiveAssistantWindow();
          if (assistantKind === 'written' && assistantWindow) {
            assistantWindow.webContents.send('global-shortcut', { action: 'captureWrittenScreen' });
          } else if (assistantKind === 'interview') {
            if (await hasWrittenQuota()) {
              this.windowHelper.openAssistantWindow('written', 'captureWrittenScreen');
            } else if (assistantWindow && !assistantWindow.isDestroyed()) {
              assistantWindow.webContents.send('global-shortcut', { action: 'writtenQuotaExhausted' });
            }
          } else {
            await this.captureScreenAndProcess();
          }
        } else if (actionId === 'general:capture-leetcode') {
          const assistantKind = this.windowHelper.getActiveAssistantKind();
          const assistantWindow = this.windowHelper.getActiveAssistantWindow();
          if (assistantKind === 'written' && assistantWindow) {
            assistantWindow.webContents.send('global-shortcut', { action: 'captureWrittenLeetCode' });
          } else if (assistantKind === 'interview') {
            if (await hasWrittenQuota()) {
              this.windowHelper.openAssistantWindow('written', 'captureWrittenLeetCode');
            } else if (assistantWindow && !assistantWindow.isDestroyed()) {
              assistantWindow.webContents.send('global-shortcut', { action: 'writtenQuotaExhausted' });
            }
          }
        } else if (
          actionId === 'general:mobile-written-scroll-up'
          || actionId === 'general:mobile-written-scroll-down'
        ) {
          // Phone paging is a companion-session action, not a written-window
          // action. Route it through whichever Lite renderer is alive so it
          // continues to work from Home, Interview, or a temporarily stale
          // assistant-kind state.
          const targetWindow = this.windowHelper.getActiveAssistantWindow() ?? this.getMainWindow();
          if (targetWindow && !targetWindow.isDestroyed()) {
            targetWindow.webContents.send('global-shortcut', {
              action: actionId.endsWith('-up') ? 'scrollMobileWrittenUp' : 'scrollMobileWrittenDown',
            });
          }
    ...
    if (!LITE_MODE) {
      ...
      console.log('[AppState] Lite mode: RAG and reference-file indexing disabled');
    }
  ...
  private sendToMeetingSurfaces(channel: string, ...args: any[]): void {
    ...
    this.windowHelper.getAssistantWindows().forEach(sendOnce);
  }
  ...
  /** Push a transcript payload to every active interview surface. */
  private emitTranscriptToSurfaces(payload: { speaker: string; text: string; timestamp: number; final: boolean; confidence: number; segmentId?: string; speechEnded?: boolean }): void {
    const helper = this.getWindowHelper();
    helper.getLauncherWindow()?.webContents.send('native-audio-transcript', payload);
    helper.getOverlayWindow()?.webContents.send('native-audio-transcript', payload);
    helper.getAssistantWindows().forEach((win) => win.webContents.send('native-audio-transcript', payload));
  }
   ...
   * chatty STT doesn't generate near-per-token IPC to two windows. Keying by
   * both speaker and segment is important: a final for segment B must never
   * cancel an unsent partial for segment A, otherwise the beginning of a long
   * question can disappear before the renderer receives it.
   */
  private sendThrottledTranscript(payload: { speaker: string; text: string; timestamp: number; final: boolean; confidence: number; segmentId?: string; speechEnded?: boolean }): void {
    const key = `${payload.speaker}:${payload.segmentId || '__active__'}`;
  ...
  private setupAutoUpdater(): void {
    // Test-only state injection lets the real Electron renderer exercise the
    // update CTA without contacting or mutating the production update feed.
    const testVersion = !app.isPackaged
      ? String(process.env.MIAODA_TEST_UPDATE_VERSION || '').trim().slice(0, 64)
      : ''
    if (testVersion) {
      this.updateAvailable = true
      this.updateDownloadState = 'available'
      this.updateVersion = testVersion
      const timer = setTimeout(() => {
        this.broadcast('update-available', { version: testVersion, testOnly: true })
        this.emitUpdateState()
      }, 100)
      timer.unref?.()
      console.log(`[AutoUpdater] Test update state enabled for ${testVersion}`)
      return
    }

    if (!app.isPackaged) {
      console.log('[AutoUpdater] Development mode: automatic update checks are disabled')
      return
    }
    ...
    autoUpdater.on("checking-for-update", () => {
      ...
      if (!this.updateAvailable && this.updateDownloadState !== 'downloading' && this.updateDownloadState !== 'downloaded') {
        this.updateDownloadState = 'checking'
      }
    ...
    autoUpdater.on("update-available", async (info) => {
      ...
      this.updateVersion = String(info.version || '').slice(0, 64) || null
    ...
    autoUpdater.on("error", (err) => {
      ...
      this.updateLastError = String(errorMessage).slice(0, 240)
    ...
    autoUpdater.on("download-progress", (progressObj) => {
      ...
      this.updateDownloadPercent = Math.max(0, Math.min(100, Number(progressObj.percent) || 0))
      this.updateLastError = null
    ...
    autoUpdater.on("update-downloaded", (info) => {
      ...
      this.updateDownloadPromise = null
      this.updateVersion = String(info.version || this.updateVersion || '').slice(0, 64) || null
    ...
    // Check silently after startup and periodically thereafter. The renderer
    // remains empty until update-available fires; downloading never starts
    // until the user clicks the visible CTA.
    const check = () => {
      void this.checkForUpdates().catch(err => {
        console.error('[AutoUpdater] Automatic update check failed:', err)
      })
    }
    const firstCheckTimer = setTimeout(check, UPDATE_CHECK_DELAY_MS)
  ...
  private emitUpdateState(): void {
    this.broadcast('app-update-state', this.getUpdateState())
  }

  public getUpdateState(): AppUpdateSnapshot {
    return {
      status: this.updateDownloadState,
      currentVersion: app.getVersion(),
      ...(this.updateVersion ? { version: this.updateVersion } : {}),
      ...(this.updateDownloadState === 'downloading' || this.updateDownloadState === 'downloaded'
        ? { percent: this.updateDownloadPercent }
        : {}),
      ...(this.updateLastError ? { error: this.updateLastError } : {}),
      canAutoUpdate: canAutoInstall(),
    }
  }
  ...
  public async checkForUpdates(): Promise<void> {
    if (!this.autoUpdaterEnabled) {
      console.log('[AutoUpdater] Update check skipped because the updater is not enabled')
      return
    }
  ...
  public async downloadUpdate(): Promise<void> {
    ...
    try {
      ...
      this.updateDownloadState = this.updateAvailable ? 'available' : 'idle'
      this.updateDownloadPromise = null
      this.updateLastError = String(err?.message || err || 'Download failed').slice(0, 240)
      this.emitUpdateState()
      throw err
    }
  }
  ...
  private createSTTProvider(speaker: 'interviewer' | 'user'): STTProvider | null {
    ...
    try {
      const { MiaodaAuthClient } = require('./services/MiaodaAuthClient');
      const miaodaClient = MiaodaAuthClient.getInstance();
      if (miaodaClient.getState()?.isAuthenticated) {
        const model = miaodaClient.getAsrModel();
        console.log(`[Main] Using authenticated MiaodaStreamingSTT (${model}) for ${speaker}`);
        stt = new MiaodaStreamingSTT(model);
      }
    } catch (error) {
      console.warn('[Main] Failed to initialize authenticated Miaoda ASR:', error);
    }
    ...
    if (stt) {
    ...
    } else if (sttProvider === 'groq' || sttProvider === 'zhipu' || sttProvider === 'azure' || sttProvider === 'ibmwatson') {
      ...
      } else if (sttProvider === 'zhipu') {
        try {
          const { MiaodaAuthClient } = require('./services/MiaodaAuthClient');
          const state = MiaodaAuthClient.getInstance().getState();
          if (state?.isAuthenticated) {
            modelOverride = SettingsManager.getInstance().get('miaodaAsrModel') || 'fun-asr-realtime';
            console.log(`[Main] Using MiaodaStreamingSTT (${modelOverride}) for ${speaker}`);
            stt = new MiaodaStreamingSTT(modelOverride);
          }
        } catch (error) {
          console.warn('[Main] Failed to inspect Miaoda auth state for server ASR:', error);
        }
        if (!stt && !apiKey) {
          apiKey = CredentialsManager.getInstance().getZhipuSttApiKey();
        }
        if (!stt && !modelOverride) {
          modelOverride = 'glm-asr-2512';
        }
      ...
      if (stt) {
        // MiaodaStreamingSTT is already constructed above because it authenticates with
        // the logged-in card token and streams through the Java backend.
      } else if (apiKey) {
    ...
    stt.on('transcript', (segment: { text: string, isFinal: boolean, confidence: number, speakerId?: string, segmentId?: string, speechEnded?: boolean }) => {
      ...
      // Lite assistants own their answer lifecycle in LiteOverlay. Feeding the
      // same final into IntelligenceManager would start a second, hidden
      // auto-answer path before Lite's VAD-confirmed question boundary.
      if (!this.currentMeetingIsLite) {
      ...
      const payload = {
        ...
        final: segment.isFinal,
        confidence: segment.confidence,
        ...(typeof segment.speechEnded === 'boolean' ? { speechEnded: segment.speechEnded } : {})
      };
    ...
    stt.on('error', (err: Error) => {
      ...
      const isAuthError = httpStatus === 401
        || /\b401\b|unauthori[sz]ed|invalid[_ -]?token|token.*(?:invalid|expired|revoked)/i.test(err.message || '')
      ...
      if (isAuthError) {
        ...
        _lastState = 'failed';
        // Authentication errors cannot recover by reconnecting the same WebSocket.
        // Stop the provider so an invalid session does not hammer the backend and
        // grow the UI counter indefinitely (previously observed at 50+ retries).
        stt.stop();
        this.sendSttStatus( {
          state: 'failed',
          provider: sttProvider,
          error: '登录状态已失效，请返回主页重新登录',
          channel: speaker,
        } as SttStatusPayload);
        return;
      }
    ...
    // Every streaming provider emits `connected` after its WebSocket is ready.
    // Clear a stale reconnecting state immediately instead of waiting for the
    // interviewer to produce another final transcript.
    stt.on('connected', () => {
      _consecutiveErrors = 0;
      if (_lastState !== 'connected') {
  ...
  private wireSystemCapture(capture: SystemAudioCapture, label: string = ''): void {
    ...
    capture.on('speech_ended', () => {
      ...
      this.sendToMeetingSurfaces('native-audio-speech-ended', {
        speaker: 'interviewer',
        timestamp: Date.now(),
      });
    });
    capture.on('speech_started', () => {
      this.sendToMeetingSurfaces('native-audio-speech-started', {
        speaker: 'interviewer',
        timestamp: Date.now(),
      });
    });
  ...
  private async setupSystemAudioPipeline(includeMicrophone = true): Promise<void> {
    ...
    try {
      ...
      if (screenCapability.effectiveDenied) {
        ...
        try {
          ...
          this.sendAudioCaptureFailed({
            channel: 'system',
            message: 'System audio capture failed to initialize. The native audio module could not allocate the capture device. Restarting 秒答 may help; if the problem persists, file a bug.',
            attempt: 0,
            maxAttempts: 0,
            terminal: true,
            stuck: false,
      ...
      if (includeMicrophone && !this.microphoneCapture) {
        ...
        try {
          ...
          this.sendAudioCaptureFailed({
            channel: 'mic',
            message: 'Microphone capture failed to initialize. The native audio module could not open the default input device. Check that the device is connected and not in exclusive use by another app, then restart 秒答.',
            attempt: 0,
            maxAttempts: 0,
            terminal: true,
            stuck: false,
      ...
      if (includeMicrophone && !this.googleSTT_User) {
  ...
  private checkSameInputOutputDevice(inputId?: string, outputId?: string): string | undefined {
    ...
    // name (input IDs from cpal ARE the device name, e.g. "秒答's AirPods Pro").
    try {
   ...
   *
   * Crash/hang fix (2026-06-05): saving an STT key could fire up to TWO
   * reconfigure calls back-to-back — one from the key handler (which
   * auto-promotes the STT provider and reconfigures), and one from the
   * renderer's follow-up `set-stt-provider` call. Each call tears down and
   * rebuilds the native captures (SystemAudioCapture / MicrophoneCapture → CoreAudio /
  ...
  public async startMeeting(metadata?: any): Promise<void> {
    ...
    const liteOverlay = Boolean(metadata?.liteOverlay);
    this.currentMeetingIsLite = liteOverlay;
    ...
    if (!liteOverlay && !(await ensureMacMicrophoneAccess('meeting start'))) {
      ...
      // a denied/revoked mic grant made "Start 秒答" do nothing on screen.
    ...
    // Emit session reset to clear the legacy meeting UI state immediately.
    // Lite owns its own visible card state; resetting it here makes the first
    // Start click look like a page flash before audio has actually begun.
    if (!liteOverlay) {
    ...
    this._audioInitPromise = (async () => {
      ...
      try {
        ...
        await this.setupSystemAudioPipeline(!liteOverlay);
        if (!isCurrentMeeting()) {
          abortStaleAudioInit();
          return;
        }
        ...
        // Lite 秒答模式只识别电脑系统音频。不要启动麦克风采集或第二条
        // ASR WebSocket，避免扬声器回声造成重复问题和双倍语音计费。
        if (!liteOverlay) {
          this.microphoneCapture?.start();
          this.googleSTT_User?.start();
  ...
  public async endMeeting(): Promise<void> {
    ...
    const liteOverlay = this.currentMeetingIsLite;
    ...
    // "Meeting ongoing" pill reverts to "Start 秒答" immediately;
    // trailing transcript finals are still accepted via `_isDraining`.
    this.isMeetingActive = false;
    this.currentMeetingIsLite = false;
  ...
  public setRecognitionLanguage(key: string): void {
    ...
    // No current provider resolves 'auto' server-side, so it falls back to en-US.
  ...
  public toggleMainWindow(): void {
    ...
    // This action is the user's explicit "hide the client" control. Physically
    // hide every Miaoda window in every mode; the global shortcut remains
    // registered by the main process and can therefore restore it.
  ...
  private createScreenshotCaptureSession(
    ...
    return {
      ...
      assistantKind: this.windowHelper.getActiveAssistantKind(),
  ...
  private restoreWindowsAfterScreenshot(session: ScreenshotCaptureSession): void {
    ...
    if (shouldRestoreMainWindow) {
      if (session.assistantKind) {
        this.windowHelper.openAssistantWindow(session.assistantKind);
      } else if (session.windowMode === 'overlay') {
  ...
  public async takeScreenshot(restoreFocus: boolean = true, hideWindows: boolean = true): Promise<string> {
    return this.withScreenshotCaptureSession('full', restoreFocus, hideWindows, (session) =>
  ...
  private async captureScreenAndProcess(): Promise<void> {
    const screenshotPath = await this.takeScreenshot(false, false);
  ...
  public async takeSelectiveScreenshot(restoreFocus: boolean = true): Promise<string> {
    return this.withScreenshotCaptureSession('selective', restoreFocus, true, async () => {
  ...
  public updateTrayMenu() {
    ...
    this.tray.setToolTip('班会解答 Lite');
    ...
    const toggleAccel = toggleKb || 'CommandOrControl+Shift+M';
  ...
  private _enforceDockState(
    ...
    if (shouldApply) {
      if (wantUndetectable) {
        ...
        // Keep focus on our own window (win.focus(), not app.focus()) so
        // dock.hide()'s implicit app-deactivation doesn't hand control to the
        // app behind us.
        if (appWasFocused && targetFocusWindow && !targetFocusWindow.isDestroyed()) {
  ...
  private _applyDisguise(mode: 'terminal' | 'settings' | 'activity' | 'none'): void {
      ...
      default:
        ...
        if (isMac) {
          iconPath = app.isPackaged
            ...
            : path.join(app.getAppPath(), "assets/miaoda.icns");
        } else if (isWin) {
          iconPath = app.isPackaged
    ...
    if (isWin) {
      ...
      app.setAppUserModelId(`org.miaoda.opensource.disguise.${mode}`);
    }
...
async function initializeApp() {
  ...
  // while 秒答 is running), focus and recenter the existing window so the
  ...
  // "秒答" icon multiple users reported. We therefore drop to 'accessory'
  ...
  if (!MIAODA_LIGHTWEIGHT) OllamaManager.getInstance().init().catch(console.error);
...
if (process.env.THINKING_MATRIX === '1') {
  ...
  // tile is visible, so macOS never paints a second "秒答" icon.
  ...
  if (!MIAODA_LIGHTWEIGHT && SettingsManager.getInstance().get('phoneMirrorEnabled')) {
  ...
  //   1. The 秒答 launcher window is visible and focused when the TCC dialog
      ...
      try {
        ...
        try {
          ...
          if (micStatus === 'denied') {
            ...
            appState.sendAudioCaptureFailed({
              channel: 'mic',
              message: 'Microphone is restricted by device policy. Contact your administrator to enable microphone access for 秒答.',
              attempt: 0,
              maxAttempts: 0,
              terminal: true,
              stuck: false,
