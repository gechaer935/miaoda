...
function containsAudiblePcm16(chunk: Buffer): boolean {
    // Native silence keepalives are zero-filled PCM16. Requiring several
    // samples above the floor avoids treating an isolated click as speech while
    // still detecting the first real frame (including a preroll batch).
    let audibleSamples = 0;
    for (let offset = 0; offset + 1 < chunk.length; offset += 2) {
        if (Math.abs(chunk.readInt16LE(offset)) >= 48 && ++audibleSamples >= 3) return true;
    }
    return false;
}
...
export class SystemAudioCapture extends EventEmitter {
    ...
    private waitingForSpeechStart: boolean = false;
    ...
    public start(): void {
        ...
        try {
            ...
            this.chunkCount = 0;
            // Detect the initial utterance as well as every resume after a
            // speech-ended edge. This lets the renderer suppress text-gap
            // fallbacks for a minute-long continuous question.
            this.waitingForSpeechStart = true;
            ...
            this.monitor.start((err: Error | null, chunk: Buffer) => {
                ...
                if (chunk && chunk.length > 0) {
                    ...
                    if (this.waitingForSpeechStart && containsAudiblePcm16(chunk)) {
                        this.waitingForSpeechStart = false;
