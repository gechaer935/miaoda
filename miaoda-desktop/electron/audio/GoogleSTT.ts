...
export class GoogleSTT extends EventEmitter {
    ...
    // Diagnostic raw-PCM dump. Opt-in via MIAODA_STT_DUMP=1. Captures the
    ...
    private openDumpStream(): void {
        if (process.env.MIAODA_STT_DUMP !== '1' || this.dumpStream) return;
        try {
    ...
    public write(audioData: Buffer): void {
        ...
        // keepalives. Google's streamingRecognize (unlike Deepgram/秒答, which
