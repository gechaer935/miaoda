...
test('AppState sends model-changed only to current model listeners', () => {
  ...
  assert.ok(/getOverlayWindow\s*\(\s*\)/.test(helperBody), 'BUG: sendModelChanged must target the overlay, where LiteOverlay listens.');
...
test('model-changed call sites use the targeted helper', () => {
  ...
  for (const needle of ['set-miaoda-api-key', "safeHandle('set-model'", "safeHandle('set-default-model'"]) {
