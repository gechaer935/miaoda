...
// the SAME audio devices produced correct transcripts on Deepgram/秒答.
//
...
// keepalives. Deepgram/秒答 endpoint cleanly on the zero frames; Google's
...
const DUMMY_KEY = path.join(os.tmpdir(), `miaoda-stt-test-sa-${process.pid}.json`);
fs.writeFileSync(
  DUMMY_KEY,
  JSON.stringify({
    type: 'service_account',
    project_id: 'miaoda-stt-test',
    private_key_id: 'test',
    // Ephemeral test key, generated locally; no committed private key material.
    private_key: generateKeyPairSync('rsa', { modulusLength: 2048 }).privateKey.export({ type: 'pkcs8', format: 'pem' }),
    client_email: 'test@miaoda-stt-test.iam.gserviceaccount.com',
    client_id: '0',
