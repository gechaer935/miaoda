 ...
 *
 * Dual-channel architecture: 秒答 captures Mic and System Audio as two
...
export class LocalWhisperSTT extends EventEmitter {
    ...
    setRecognitionLanguage(key: string): void {
        ...
        console.log(`[LocalWhisperSTT] recognition language for ${this.channelLabel || 'channel'} set to: ${this.language}`);
    }
    ...
    private handleStreamingPartial(text: string): void {
        ...
        const cleaned = filterHallucination(text, { expectedLanguage: this.language });
        if (!cleaned) return;
    ...
    private attachWorkerListeners(): void {
        ...
        this.worker.on('message', (msg: WorkerOutMessage) => {
            ...
            if (msg.type === 'partial') {
                ...
                const text = filterHallucination(msg.text, { expectedLanguage: this.language });
                if (text) {
