...
const LANG_MAP: Record<string, string | null> = {
  'auto': null,
  // Internal keys from electron/config/languages.ts.
  'english-us': 'en',
  'english-uk': 'en',
  'english-in': 'en',
  'english-au': 'en',
...
const CHINESE_PROMPT = [
  '以下是普通话中文技术面试或会议转写。',
  '请使用简体中文记录问题和回答，保留英文技术名词。',
  '常见术语包括：分布式锁、分布式死锁、死锁、锁、事务、数据库、缓存、索引、一致性、可用性、分区容错、恢复、故障、线程、进程、并发。'
].join('');
...
let lastLoggedTranscribeConfig = '';
...
parentPort.on('message', async (msg: any) => {
  if (msg.type === 'init') {
    ...
    try {
      ...
      if (language === 'zh' && !cachedPromptIds && !isMoonshineModel(loadedModelId)) {
        try {
          const encoded = await pipe.tokenizer(CHINESE_PROMPT, { add_special_tokens: false });
          const raw = encoded?.input_ids?.tolist?.()?.[0] ?? [];
          cachedPromptIds = raw.slice(0, PROMPT_TOKEN_CAP).map((n: bigint | number) => Number(n));
          cachedPromptText = CHINESE_PROMPT;
        } catch (e: any) {
          console.warn('[WhisperWorker] Chinese prompt tokenization failed:', e?.message);
        }
      }

      const configKey = `${loadedModelId}|${msg.language}|${language ?? 'auto'}|${streaming ? 'stream' : 'final'}`;
      if (configKey !== lastLoggedTranscribeConfig) {
        lastLoggedTranscribeConfig = configKey;
        console.log(`[WhisperWorker] transcribe model=${loadedModelId} appLanguage=${msg.language ?? 'unset'} whisperLanguage=${language ?? 'auto'} streaming=${streaming}`);
      }
