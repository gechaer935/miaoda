...
export interface NativeShortcutKeyInput {
  key: string;
  ctrlKey: boolean;
  altKey: boolean;
  shiftKey: boolean;
...
export interface NativeModule {
  ...
  // Windows-only physical key-state reader used while editing shortcuts. It
  // still sees combinations consumed by Windows or another application's
  // RegisterHotKey before Chromium receives a keydown event.
  getPressedShortcutKey?: () => NativeShortcutKeyInput | null;
  ...
  // window so clicks/keystrokes don't activate 秒答 (foreground app
  ...
  removeStealthFromWindow?: (handle: Buffer) => void;
 ...
 *
 * We bypass `require('miaoda-audio')` intentionally. That approach relied on
 * npm creating a symlink from node_modules/miaoda-audio -> native-module/,
