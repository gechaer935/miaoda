    ...
    try {
      ...
      if (sink.name === 'posthog') {
        ...
        post(`${host}/capture/`, {
          ...
          body: JSON.stringify({
            api_key: apiKey,
            event: record.name,
            distinct_id: sink.distinctId || 'miaoda-desktop',
            properties: {
              ...record.properties,
      ...
      if (sink.name === 'sentry') {
        ...
        const event = {
          ...
          platform: 'node',
          level: 'error',
          logger: 'miaoda-desktop',
          release: sink.release || 'unknown',
          environment: sink.environment || 'production',
          tags: { service: 'miaoda-desktop', event: record.name, ...(record.provider ? { provider: record.provider } : {}) },
...
export function parseClientSentryDsn(dsn: string | undefined): ParsedSentryDsn | null {
  ...
  try {
    ...
    return {
      ...
      authHeader: `Sentry sentry_version=7, sentry_client=miaoda-desktop/1.0, sentry_key=${publicKey}`,
    };
  } catch {
    return null;
  }
