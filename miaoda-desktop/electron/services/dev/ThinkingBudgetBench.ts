...
export async function runThinkingMatrix(llmHelper: LLMHelper, opts: { model?: string; log?: (s: string) => void; delayMs?: number; configs?: string } = {}): Promise<any> {
  ...
  // THINKING_BENCH_KEYS_FROM_ENV=1, load every GEMINI_API_KEY* from
  // miaoda-api/.env, build a client per key, rotate per call, and on a 429
  // retry on the NEXT key after a short backoff. Falls back to the app client.
  const clients: any[] = [];
  if (process.env.THINKING_BENCH_KEYS_FROM_ENV === '1') {
    try {
      const { GoogleGenAI } = require('@google/genai');
      const envPath = path.join(process.cwd(), 'miaoda-api/.env');
      ...
      log(`[matrix] rotating across ${clients.length} keys from miaoda-api/.env${only.length ? ` (indices ${only.join(',')})` : ''}`);
