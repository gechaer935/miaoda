...
test('disk-write failure is reported (success=false), never a false "Saved"', () => {
  ...
  // Windows does not enforce POSIX chmod write bits. Inject the same filesystem
  // failure at the write boundary so the error path is deterministic on all OSes.
  const originalWriteFileSync = fs.writeFileSync;
  fs.writeFileSync = (target, ...args) => {
    if (String(target).startsWith(env.userData)) {
      const error = new Error('synthetic filesystem write failure');
      error.code = 'EACCES';
      throw error;
    }
