...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'miaoda-telemetry-test-'));
}
...
test('API keys and tokens are redacted from properties', () => {
  const sanitized = sanitizeTelemetryProperties({
    ...
    nested: {
      token: 'miaoda_sk_supersecretvalue',
  ...
  assert.doesNotMatch(serialized, /miaoda_sk_supersecretvalue/);
