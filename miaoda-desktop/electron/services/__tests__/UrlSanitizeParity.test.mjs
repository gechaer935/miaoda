...
// primary (sanitizeUrl in miaoda-browser tab-classifier.ts) and the desktop
...
const extRoot = path.join(repoRoot, 'miaoda-browser');
...
const FIXTURES = [
  ...
  'https://x.com/auth/eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ4In0AbCdEf.s5kT9QabcdefGhIJ', // realistic JWT // gitleaks:allow -- fabricated URL-redaction fixture, not usable credentials.
  'https://app.acme.com/reset/AbCdEfGhIjKlMnOpQr',  // short (18) embedded token — final-review HIGH
  'https://acme.com/p/sk_live_51HxYzAbCdEfGhIj',     // underscore key — final-review HIGH // gitleaks:allow -- fabricated URL-redaction fixture, not usable credentials.
...
describe('URL sanitizer parity — extension sanitizeUrl === desktop reSanitizeUrl', () => {
  ...
  test('FINAL-REVIEW HIGH: short embedded token + underscore key + opaque subdomain are redacted in BOTH copies', () => {
    for (const [input, expected] of [
      ...
      ['https://acme.com/p/sk_live_51HxYzAbCdEfGhIj', 'https://acme.com/p/:token'], // gitleaks:allow -- fabricated URL-redaction fixture, not usable credentials.
