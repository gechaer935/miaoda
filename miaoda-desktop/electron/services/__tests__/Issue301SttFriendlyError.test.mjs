...
test('LiteOverlay uses sttErrorMapper for the chat-message STT error', () => {
  ...
  assert.match(
    ...
    'categorizeSttError must be statically imported in LiteOverlay.tsx',
  );
...
test('sttErrorMapper auth category covers all MiaodaPro fatal error codes', () => {
  const source = read('src/lib/sttErrorMapper.ts');
  // These are the fatal codes streaming STT providers emit that cause state='failed'
