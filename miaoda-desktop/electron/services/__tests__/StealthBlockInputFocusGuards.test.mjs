...
// src/components/LiteOverlay.tsx — the heart of PR #250 (issue #246,
...
//
//   1. blockInputFocus (src/components/LiteOverlay.tsx ~line 3281)
...
const MIAODA_INTERFACE = path.join(
  root,
  'src/components/LiteOverlay.tsx',
);
...
function shouldBlockFocus(refs) {
  // Mirrors blockInputFocus in LiteOverlay.tsx (~line 3281).
...
function shouldFireStealthTapStart(refs) {
  // Mirrors the mount-effect onMouseDown in LiteOverlay.tsx (~line 3223).
...
describe('LiteOverlay.tsx: guard implementation must keep checking both refs', () => {
  const source = fs.readFileSync(MIAODA_INTERFACE, 'utf8');
