...
// OcrProviderManager. OCR has been removed from 秒答's runtime
...
// =====================================================================
// Set MIAODA_RUN_LEGACY_OCR_TESTS=1 to opt in.
...
const LEGACY_OCR_ENABLED = process.env.MIAODA_RUN_LEGACY_OCR_TESTS === '1';
const test = (name, opts, fn) => {
  if (typeof opts === 'function') return _test(name, { skip: LEGACY_OCR_ENABLED ? undefined : 'legacy OCR path disabled — set MIAODA_RUN_LEGACY_OCR_TESTS=1 to run' }, opts);
  return _test(name, { ...(opts || {}), skip: LEGACY_OCR_ENABLED ? undefined : 'legacy OCR path disabled — set MIAODA_RUN_LEGACY_OCR_TESTS=1 to run' }, fn);
};
