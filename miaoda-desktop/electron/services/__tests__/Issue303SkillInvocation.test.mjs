...
// ---------------------------------------------------------------------------
// 4. Skill picker UI wiring (static source assertions on LiteOverlay.tsx)
// ---------------------------------------------------------------------------
describe('Issue #303: skill picker dropdown in overlay chat input', () => {
  test('createPortal is imported from react-dom in LiteOverlay.tsx', () => {
    const source = read('src/components/LiteOverlay.tsx');
    assert.match(source, /import\s*\{\s*createPortal\s*\}\s*from\s*['"]react-dom['"]/,
      'createPortal must be imported from react-dom');
  });

  test('SkillPicker component is defined in LiteOverlay.tsx', () => {
    const source = read('src/components/LiteOverlay.tsx');
  ...
  test('skill list is fetched on mount via skillsRefresh in LiteOverlay.tsx', () => {
    const source = read('src/components/LiteOverlay.tsx');
    assert.match(source, /skillsRefresh\?\.\(\)/, 'skillsRefresh must be called in LiteOverlay');
