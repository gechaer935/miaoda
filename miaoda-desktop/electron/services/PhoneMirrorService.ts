...
// won't match any of these exact IDs. See miaoda-browser/README.md + CONTRACT.md.
...
const PINNED_EXTENSION_IDS = new Set(
  [STORE_EXTENSION_ID, DEV_EXTENSION_ID, process.env.MIAODA_DOM_EXTENSION_ID].filter(
    (id): id is string => !!id,
  ),
);
...
export class PhoneMirrorService {
  ...
  // mounts LiteOverlay). When it yields no live window, /dom returns 409.
   ...
   * the overlay window that mounts LiteOverlay — captured page content is
   ...
   * overlay resolver (the window mounting LiteOverlay); falls back to any
      ...
      req.on('end', () => {
        ...
        try {
          ...
          if (parsed && typeof parsed.dom === 'string') {
            ...
            // Deliver to the overlay window (the one that mounts LiteOverlay).
            // No live overlay → no active session to receive context → 409, so the
            // extension can tell the user to start a 秒答 session first.
