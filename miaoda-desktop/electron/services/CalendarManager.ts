...
// exchange and refresh must be proxied through a server that holds the secret.
...
// Base URL of the OAuth token proxy. This used to default to the upstream
// vendor's host, which meant a Google authorization code and refresh token were
// POSTed to a third party we do not operate. 秒答 ships no calendar proxy, so
// there is no default: the exchange fails closed unless an operator points
// CALENDAR_TOKEN_PROXY_URL at a server they control.
const TOKEN_PROXY_URL = (process.env.CALENDAR_TOKEN_PROXY_URL || '').replace(/\/+$/, '');

function requireTokenProxy(): string {
    if (!TOKEN_PROXY_URL) {
        throw new Error('calendar_token_proxy_not_configured: set CALENDAR_TOKEN_PROXY_URL to a server you operate before using calendar sync');
    }
    return TOKEN_PROXY_URL;
}
...
export class CalendarManager extends EventEmitter {
            ...
            const server = http.createServer(async (req, res) => {
                try {
                    if (req.url?.startsWith('/auth/callback')) {
                        ...
                        if (code) {
                            res.end('Authentication successful! You can close this window and return to 秒答.');
    ...
    private async exchangeCodeForToken(code: string) {
        try {
            // Proxied so GOOGLE_CLIENT_SECRET never ships in the desktop app.
            // Fetch (vs. axios) so this call shares the global keep-alive pool and
            // exposes the same error shape (res.ok / res.status) as the rest of the codebase.
            const response = await fetch(`${requireTokenProxy()}/api/calendar/exchange`, {
                method: 'POST',
    ...
    private async refreshAccessToken() {
        ...
        try {
            // Proxied so GOOGLE_CLIENT_SECRET never ships in the desktop app.
            const response = await fetch(`${requireTokenProxy()}/api/calendar/refresh`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    private showNotification(event: CalendarEvent) {
        ...
        const notif = new Notification({
            ...
            body: `"${event.title}" starts in 2 minutes. Start 秒答?`,
            actions: [
