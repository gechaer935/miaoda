...
// Domain-separation tag mixed into the fallback key derivation. Changing it
// changes the derived key, which makes any fallback file written under a
// different tag undecryptable — treat it as frozen once real installs exist.
const FALLBACK_DOMAIN_TAG = 'miaoda-credential-fallback-v1';
...
export interface StoredCredentials {
    ...
    /** Manual output ceiling for LiteLLM-proxied models. Unset 閳?Auto (per-model via /model/info). */
    ...
    sttProvider?: 'none' | 'google' | 'groq' | 'openai' | 'zhipu' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'local-whisper';
    groqSttApiKey?: string;
    groqSttModel?: string;
    openAiSttApiKey?: string;
    zhipuSttApiKey?: string;
    /** Custom OpenAI-compatible STT base URL (e.g. self-hosted Speaches).
     *  Empty / unset 閳?use https://api.openai.com. */
    ...
    // Tavily Search
    tavilyApiKey?: string;
    // Dynamic Model Discovery 閳?preferred models per provider
    geminiPreferredModel?: string;
    groqPreferredModel?: string;
    openaiPreferredModel?: string;
    claudePreferredModel?: string;
    deepseekPreferredModel?: string;
    zhipuPreferredModel?: string;
    /**
     * Companion-extension pairing token. LOOPBACK-SCOPED 閳?only the extension uses
...
export class CredentialsManager {
    ...
    public init(): void {
        ...
        // macOS/Windows. Metadata only 閳?never key contents.
        this.emitStorageStatusDiagnostic('startup');
    }

    /**
     * Emit a privacy-safe snapshot of OS secure-storage availability via the
     * shared TelemetryService. Carries ONLY booleans/enums/platform 閳?never any
     ...
     *
     * Fields:
     *  - available:   safeStorage.isEncryptionAvailable() 閳?false 閳?keys won't survive restart
     *  - platform:    process.platform (darwin/win32/linux)
     *  - backend:     (linux only) safeStorage.getSelectedStorageBackend() 閳?the
     *                 key signal: 'basic_text' 閳?no keyring (expected failure),
     *                 'gnome_libsecret'/'kwallet*' 閳?keyring present
     *  - packaged:    app.isPackaged 閳?distinguishes the unsigned/dev-build hypothesis
     *
    ...
    public emitStorageStatusDiagnostic(phase: 'startup' | 'stt_save_failed'): void {
        try {
            ...
            // available 閳?it tells basic_text (no keyring) from gnome_libsecret/kwallet.
            if (process.platform === 'linux') {
                ...
                } catch { /* backend probe unavailable 閳?leave it off */ }
            }
    ...
    public getZhipuApiKey(): string | undefined {
        return this.credentials.zhipuApiKey || this.credentials.zhipuSttApiKey;
    }
    ...
    public getSttProvider(): 'none' | 'google' | 'groq' | 'openai' | 'zhipu' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'local-whisper' {
        return this.credentials.sttProvider || 'zhipu';
    }
    ...
    public getGroqSttModel(): string {
        return this.credentials.groqSttModel || 'whisper-large-v3-turbo'; // gitleaks:allow -- speech model identifier, not an API key.
    }
    ...
    public getZhipuSttApiKey(): string | undefined {
        return this.credentials.zhipuSttApiKey || this.credentials.zhipuApiKey;
    }
    ...
    public getSttLanguage(): string {
        return this.credentials.sttLanguage || 'chinese';
    }
    ...
    public getDefaultModel(): string {
        // Keep a conservative built-in fallback for users who have not configured
        // any provider yet. Provider-specific setters promote their own defaults.
        return this.credentials.defaultModel || 'deepseek-v4-flash';
    }
    ...
    // =========================================================================
    // Vision provider availability 閳?used by the vision-first screen pipeline
    // =========================================================================
    ...
    public anyLocalVisionProviderConfigured(): boolean {
        ...
        // Here we only assert the runtime is configured 閳?model gating happens in the chain.
        ...
        // Codex CLI is local in normal install 閳?capability is verified by ProviderRouter.
    ...
    public setZhipuApiKey(key: string): void {
        const trimmed = key.trim();
        this.credentials.zhipuApiKey = trimmed || undefined;
        ...
        if (trimmed) {
            if (!this.credentials.sttProvider
                ...
                this.credentials.sttProvider = 'zhipu';
                console.log('[CredentialsManager] Auto-set STT provider to zhipu');
            }
        } else {
            if (this.credentials.defaultModel?.startsWith('glm-')) {
                this.credentials.defaultModel = 'deepseek-v4-flash';
                console.log('[CredentialsManager] Zhipu key cleared - reset default model to DeepSeek v4');
            }
            if (this.credentials.sttProvider === 'zhipu' && !this.credentials.zhipuSttApiKey) {
                this.credentials.sttProvider = 'none';
                console.log('[CredentialsManager] Zhipu key cleared - reset STT provider to none');
            }
        }

        this.saveCredentials();
        console.log('[CredentialsManager] Zhipu API Key updated');
    }
     ...
     * writes this 閳?on first start (mint) and on Rotate token. The phone token is
     ...
     * maxTokens is the optional user-set output ceiling (0/undefined 閳?default).
    ...
    public setSttProvider(provider: 'none' | 'google' | 'groq' | 'openai' | 'zhipu' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'local-whisper'): void {
    ...
    public setZhipuSttApiKey(key: string): boolean {
        this.credentials.zhipuSttApiKey = key.trim() || undefined;
        if (this.credentials.zhipuSttApiKey) {
            this.credentials.sttProvider = 'zhipu';
        }
        const persisted = this.saveCredentials();
        console.log('[CredentialsManager] Zhipu STT API Key updated');
        return persisted;
    }
    ...
    public getPreferredModel(provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu'): string | undefined {
        const key = `${provider}PreferredModel` as keyof StoredCredentials;
        return this.credentials[key] as string | undefined;
    }

    public setPreferredModel(provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu', modelId: string): void {
        const key = `${provider}PreferredModel` as keyof StoredCredentials;
     ...
     * restart 閳?via EITHER the OS keyring (safeStorage) OR the app-managed AES
     ...
     * per install is the ONLY machine/install-binding input 閳?see getFallbackKey()
     ...
     * but a *transient* read error (EIO/EACCES on an existing file) FAILS CLOSED 閳?     * we must not regenerate a salt that would orphan a still-recoverable fallback.
     */
     ...
     *
     * IMPORTANT 閳?key-material stability: the key is derived from the per-install
     ...
     * (hostname flips with Wi-Fi/DHCP/mDNS `.lan`閳摽.local` and machine renames;
     ...
     * undecryptable 閳?silently reintroducing the very "STT key reset to none" bug
    ...
    private getFallbackKey(): Buffer {
        ...
        this.fallbackKey = deriveFallbackKey([FALLBACK_DOMAIN_TAG, process.platform], this.getOrCreateDeviceSalt());
    ...
    private saveCredentials(): boolean {
        try {
            if (safeStorage.isEncryptionAvailable()) {
                ...
                // Keyring is the source of truth now 閳?drop any stale fallback file.
                this.removeFallbackFile();
                return true;
            }

            // OS keyring unavailable 閳?use the app-managed encrypted fallback so the
        ...
        try {
            ...
            if (fs.existsSync(CREDENTIALS_PATH)) {
                ...
                if (keyringAvailable) {
                    ...
                    try {
                        ...
                        console.error('[CredentialsManager] Failed to parse decrypted credentials 閳?file may be corrupted. Starting fresh:', parseError);
                        this.credentials = {};
                    }
                    // Keyring is authoritative 閳?clean up any stale fallback + plaintext.
                    this.removeFallbackFile();
            ...
            if (fs.existsSync(FALLBACK_PATH)) {
                try {
                    ...
                    console.error('[CredentialsManager] Failed to read app-managed fallback 閳?starting fresh:', fbErr);
                    this.credentials = {};
                }
                ...
                if (keyringNow && Object.keys(this.credentials).length > 0) {
                    console.log('[CredentialsManager] Keyring now available 閳?migrating fallback credentials to keyring');
                    this.saveCredentials();
                }
                this.removePlaintextFile();
                return;
