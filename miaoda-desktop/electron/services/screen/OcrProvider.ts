...
// =====================================================================
// 秒答 now uses vision-provider screen understanding by default.
...
//
// Original purpose:
// Unified OCR provider interface for 秒答.
...
async function prepareImageForOcr(imagePath: string, maxDimension = 1600): Promise<{ path: string; cleanup?: () => Promise<void> }> {
  ...
  const tempPath = path.join(os.tmpdir(), `miaoda-ocr-${uuidv4()}.png`);
    ...
    try {
      ...
      const result = await Tesseract.recognize(
        ...
        options?.languages?.[0] || 'eng',
        {
          ...assetPaths,
          logger: (m: any) => {
            if (process.env.MIAODA_OCR_DEBUG === '1' && m.status === 'recognizing text') {
