...
//
// VISION-FIRST screen understanding pipeline for 秒答.
//
// Flow:
//   request
...
export class ScreenUnderstandingService {
  ...
  async understand(request: ScreenUnderstandingRequest): Promise<ScreenUnderstandingResult> {
    ...
    const userDataDir =
      ...
      process.env.MIAODA_TEST_USER_DATA ||
      '';
    ...
    if (providers.length === 0) {
      ...
      return this.buildUnavailable({
        ...
        unavailableReason: mode === 'private_vision'
          ...
          : 'No vision-capable provider is configured. Add an API key for OpenAI, Claude, Gemini, Groq, or 秒答, or configure a local Ollama vision model.',
      });
    }
