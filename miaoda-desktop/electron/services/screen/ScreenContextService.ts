...
// mode could be reintroduced. 秒答's default screen-understanding
...
export class ScreenContextService {
    ...
    async captureScreenFromPath(screenshotPath: string): Promise<ScreenContext> {
        ...
        try {
            ...
            const result = await ocrManager.recognize(screenshotPath, { timeoutMs: 8_000, maxDimension: 1200, languages: ['eng+chi_sim'] });
            ocrText = result.text;
    ...
    async runOCR(imagePath: string): Promise<string> {
        try {
            const ocrManager = this.getOcrManager();
            const result = await ocrManager.recognize(imagePath, { timeoutMs: 8_000, maxDimension: 1200, languages: ['eng+chi_sim'] });
            return result.text;
        } catch (error) {
