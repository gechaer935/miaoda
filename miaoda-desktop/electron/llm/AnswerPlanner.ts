...
export type AnswerType =
  ...
  // to build 秒答", "repo-verifiable code"). Must retrieve real source if
  ...
  // A question ABOUT the product/project itself ("what kind of app is 秒答?",
...
export interface AnswerPlan {
   ...
   * → "秒答"). Set for project_followup_answer; undefined otherwise. Used to
...
// 秒答 mention) on top of the shared section spec.
...
- NEVER mention "秒答", the assistant, the product, or the candidate's profile/projects anywhere in the answer — not in the explanation, not in a closing remark, not in an example. This is a pure technical answer about the algorithm only.`;
...
const INTERVIEW_TECHNICAL_TEMPLATE = `You are the candidate SPEAKING this answer aloud to an interviewer. Give the exact words you'd SAY, in the same language as the question.

For Chinese CS interview questions, use this interview-answer shape:
- Start with a direct one-sentence definition.
- Then give 2-4 concrete points: core mechanism, key conditions, tradeoffs, or differences.
- If the question asks "how to avoid / recover / optimize", add practical measures.
- Do NOT say you lack context. Generic operating-system, database, network, concurrency, and distributed-system questions should be answered from general technical knowledge.
- Do NOT coach the user on how to answer. Never write phrases like "这个问题可以按面试回答来讲", "先给定义", "展开时建议", "可以从三点回答", "如果面试官追问", "建议这样说", or their English equivalents.
- Output only the answer content that the candidate can read aloud directly.

Keep it speakable: usually 120-220 Chinese characters for a simple concept, up to 350-500 Chinese characters for a multi-part question. Short paragraphs are fine. Avoid code blocks unless the user explicitly asks for code.`;

// SALES voice (manual regression 2026-06-12): real sales-mode sessions answered
// "why is your product expensive?" with "I'm 秒答, an AI assistant. I don't
...
Rules:
- Speak in FIRST PERSON as the product's representative ("our product", "we", "our pricing"). You are NOT an AI assistant in this answer and must NEVER say "I'm 秒答", "as an AI", or "I don't have a product" — the user's product context defines the product.
...
- If the exact source code is NOT loaded, say clearly: "I don't have 秒答's exact source code loaded in my current context, so I can't give you a repo-verifiable snippet." Then, ONLY IF it helps, offer a clearly-labeled CONCEPTUAL example: prefix it with "Here's a conceptual illustration (NOT the actual repo code):".
...
const TECHNICAL_SUBJECT_PATTERNS = [
  ...
  /(锁|互斥锁|读写锁|自旋锁|乐观锁|悲观锁|分布式锁|死锁|并发|线程|进程|协程|信号量|条件变量|竞态|原子性)/i,
  /(操作系统|内存管理|分配内存|地址空间|堆|栈|虚拟内存|物理内存|页表|缺页|页面置换|用户态|内核态|系统调用|中断|调度|上下文切换)/i,
  /(select|poll|epoll|IO|I\/O|多路复用|Reactor|Proactor|Socket|TCP|UDP|HTTP|网络事件驱动)/i,
  /(事务|索引|隔离级别|MVCC|行锁|表锁|间隙锁|意向锁|缓存|Redis|消息队列|Kafka|一致性|分布式|CAP|Raft|Paxos)/i,
];
...
export const isStealthEvasionQuestion = (question: string): boolean => {
  ...
  // "make it invisible", "keep miaoda undetectable", "hide the overlay".
  if (hasIntent && /\b(it|miaoda|秒答|the (?:app|tool|overlay|window|ui)|this)\b/.test(t)
  ...
  if (/\b(use|using|run|running)\s+(it|this|miaoda|秒答|the (?:app|tool|overlay))\b/.test(t)
...
// "a snippet you used to build 秒答", "repo-verifiable code", "actual code
...
const SOURCE_CODE_EVIDENCE_PATTERNS = [
  // "actual/real/exact code ... of 秒答 / your repo / the codebase / github" —
  ...
  /\b(actual|real|exact|repo[- ]?verifiable|github[- ]?verifiable)\s+(code|snippet|implementation|function|source)\b.{0,50}\b(miaoda|秒答|your (repo|codebase|source|project)|the (repo|codebase|source|project)|github|gitlab)\b/i,
  /\b(miaoda|秒答|your (repo|codebase|source|project)|the (repo|codebase)|github)\b.{0,50}\b(actual|real|exact|repo[- ]?verifiable)\s+(code|snippet|implementation|function|source)\b/i,
  /\b(snippet|code|function|implementation|file)\b.{0,40}\b(you (used|wrote|built|made)|from (your|the) (codebase|repo|repository|source|github|project)|to (build|built) miaoda)\b/i,
  // "what does your actual <X> code look like", "show me your <X> code", "your
  // real code for <X>" — asking about 秒答's own implementation. A source-
  // evidence request (must not fabricate), not a generic coding task.
  /\b(what does |show me |whats )?(your|the miaoda|miaoda'?s)\s+(actual\s+|real\s+)?[\w ]*\bcode\b\s*(look|is|for|of)?/i,
  ...
  // "paste/show/give a snippet from the miaoda repo/codebase/source"
  /\b(paste|show|give|share|pull)\b.{0,30}\b(snippet|code|function|file)\b.{0,30}\b(from (the )?(miaoda|秒答) (repo|codebase|source|project)|from (your|the) (repo|codebase|github))\b/i,
  /\bsnippet from (the )?(miaoda|秒答|your|the) (repo|codebase|source|project|github)\b/i,
  ...
  /\b(show|give|write|share)\b.{0,40}\b(code|snippet)\b.{0,40}\b(you (used|wrote)|to (build|built)|from miaoda|actual|real|repo|github)\b/i,
  /\bdemo code of a snippet you have used\b/i,
  /\b(exact|actual) code from (file|the file|your)\b/i,
  // "write/give a demo snippet FOR 秒答" / "demo code for miaoda" — a request
  // for code OF the loaded project (even with a write-verb, the "for 秒答"
  ...
  /\b(write|give|show|share|make)\b.{0,30}\b(demo |sample |example )?(code|snippet)\b.{0,20}\b(for|of|from)\s+(miaoda|秒答|your project|the project)\b/i,
  /\b(demo|sample|example)\s+(code|snippet)\s+(for|of|from)\s+(miaoda|秒答|the miaoda)\b/i,
...
// "what kind of app is 秒答?", "how's its backend?", "what do you think about
// 秒答?", "what tech does it use?". Grounded in loaded project metadata.
...
const PRODUCT_ABOUT_PATTERNS = [
  ...
  /\bhow(?:'?s| is| does)\s+(miaoda|秒答|it|the (app|product|backend|architecture|frontend|stack))\b/i,
  /\bwhat\s+(do you think about|about)\s+(miaoda|秒答)\b/i,
  /\bwhat (tech|technolog|stack|languages?|framework)\w*\s+(does|do)\s+(miaoda|秒答|it|this)\b/i,
  /\bis (miaoda|秒答|it|this)\s+(local|cloud|open[- ]?source|privacy|low[- ]?distraction|on[- ]?device|transparent|accessib)\w*/i,
  /\b(miaoda|秒答)'?s\s+(backend|architecture|stack|frontend|core)\b/i,
  ...
  /\b(does|do)\s+(it|this|miaoda|秒答)\s+(process|run|store|work|use|have|support|need)\b/i,
  /\b(what|which) part (of (miaoda|秒答|it|the app))?\s*(uses|is in|runs|handles|does)\b|\b(does|do) (it|miaoda) (use|have) (a )?(backend|server|database|ollama|rust|electron|local)\b/i,
  ...
  /\bwhat (does|do) (miaoda|秒答|it) use\b|\bwhat'?s (miaoda|秒答|it) (built|made|written) (with|in)\b/i,
  // Architecture / build-stack questions ABOUT the product: "what is 秒答 built
  // with", "what is it made using", "what are the technologies behind 秒答",
  // "what is the architecture of 秒答", "how did you build 秒答" (release
  ...
  /\bwhat (is|'?s|are) (the )?(tech ?(stack)?|technolog\w*|stack|architecture|framework\w*)\s+(of\s+|behind\s+|powering\s+)?(miaoda|秒答|it|this|the (app|product|project))\b/i,
  /\b(miaoda|秒答|it|this)\s+(is\s+)?(built|made|written|developed|created|powered)\s+(with|using|in|on)\b/i,
  /\bwhat (is|'?s) (it|miaoda|秒答)\s+(made|built|written|developed)\s+(of|with|using|in)\b/i,
  /\b(what is|whats|describe) (the )?architecture (of )?(miaoda|秒答|it|this|the (app|product|project))\b/i,
  /\bhow (did|do|was) (you|miaoda|it|this)\s+(build|built|develop\w*|architect\w*|design\w*)\s+(miaoda|it|this|the (app|product))\b/i,
  /\bhow (is|was) (miaoda|秒答|it|this) (built|made|developed|architected|designed)\b/i,
  // "how (do you make|to make) it low-distraction / privacy-first / local" — a
  // product-design question about 秒答, grounded in metadata (1000-q
  // benchmark 2026-06-06b). NOT a stealth ask (no evasion/interview object).
  /\bhow (do you |to )?(make|keep|design)\s+(it|miaoda|this)\s+(low[- ]?distraction|privacy[- ]?first|private|transparent|accessible|local|on[- ]?device|minimal)\b/i,
  /\b(low[- ]?distraction|privacy[- ]?first)\b.{0,30}\b(mode|design|approach|first)\b|\bkeep (it|miaoda|this) (low[- ]?distraction|privacy)/i,
  ...
  /\bhow (to|do i|should i) disclose (it|miaoda|this|using it)\b|\bdisclose (it|miaoda|this) (in|during|to)\b/i,
  /\bmake (it|miaoda|this) accessible\b|\baccessible (without|but not) (being )?distract\w*/i,
];
...
const JD_FIT_PATTERNS = [
  ...
  /\b(why|how) (is|does) (miaoda|this project|that project|your project|it)\b.*\b(relevant|prove|matter|fit|qualify|show)\b.*\b(analyst|data|role|job)?/i,
...
const TECHNICAL_CONCEPT_PATTERNS = [
  ...
  /(什么是|什么叫|解释一下|讲一下|说一下|介绍一下|区别|有什么区别|有哪些|如何避免|怎么避免|如何恢复|为什么|原理|机制|优缺点|对比)/i,
  /(怎么|如何).{0,30}(分配|管理|实现|工作|运行|调度|通信|同步|切换)/i,
...
const PROJECT_PATTERNS = [
  ...
  // "Tell me about 秒答", "explain 秒答", "what is 秒答", "talk about
  // 秒答" — direct asks about a named project (benchmark 2026-06-05). The
  // known project entity is resolved at runtime; here we recognise the intent.
  /\b(tell me about|talk about|explain|describe|walk me through|what(?:'s| is)?)\s+miaoda\b/i,
...
const EXPERIENCE_PATTERNS = [
  ...
  /\bwhat do you think (about|of)\s+(?!the\b|this\b|that\b|your\b|my\b|it\b|all\b|everything\b|us\b|them\b|miaoda|秒答|the (role|job|company|position|team))[a-z][\w-]{3,}\b/i,
...
const classifyStandaloneFragment = (text: string): AnswerType | null => {
  ...
  if (VOICE_CONTROL_RE.test(t) || EVIDENCE_CONTROL_RE.test(t)) {
    ...
    if (/\b(project|built|miaoda|metric|impact|result)\b/i.test(t)) return 'project_answer';
...
const templateFor = (answerType: AnswerType): string => {
  switch (answerType) {
    ...
    case 'technical_concept_answer':
      ...
      return INTERVIEW_TECHNICAL_TEMPLATE;
...
// ("how is 秒答 developed?" → "秒答", "what was your role in SQL-Copilot?"
...
const classifyUnmatchedFallback = (text: string, input: PlanAnswerInput): AnswerType => {
  ...
  if (/\b(project|built|build|developed|miaoda|app|system|architecture|stack|backend|database)\b/i.test(text)) return 'project_answer';
...
const SMS_NORMALIZATIONS: Array<[RegExp, string]> = [
  ...
  [/\bexperince\b/gi, 'experience'], [/\bnativley\b/gi, 'miaoda'], [/\bnativly\b/gi, 'miaoda'],
];
...
export const planAnswer = (input: PlanAnswerInput): AnswerPlan => {
  ...
  // EXCLUDE when the object is the named product 秒答 ("how did you build
  // 秒答" is a product-about/architecture question, not a generic skill probe).
  const asksAboutProduct = /\bmiaoda\b|秒答/i.test(text);
  ...
  const isExplicitExperienceProbe = !hasWriteCodeVerb && !asksAboutProduct && !asksAboutProjectsList && !isProjectDrillIn && !hasPeopleObject && (
  ...
  if (!question) {
    ...
    answerType = 'skill_experience_answer';
             ...
             // 秒答?") OR a resolved prior-turn target makes this an
  ...
  } else if (asksAboutProduct && includesAny(text, PRODUCT_ABOUT_PATTERNS)) {
    // A question that NAMES 秒答 and is about its build/architecture/stack is a
    ...
    // 秒答" / "how did you build 秒答" route to product-about
    ...
    answerType = 'system_design_answer';
             ...
             // "I'm 秒答, an AI assistant. I don't have personal
    ...
    // "what kind of app is 秒答?", "how's its backend?", "what do you think
    // about 秒答?" — a drill-in ABOUT the product, grounded in loaded project
  ...
  const voiceLine = plan.voicePerspective === 'first_person_candidate'
    ...
    : plan.voicePerspective === 'second_person_user'
      ...
      : (plan.answerType === 'sales_answer' || plan.answerType === 'product_candidate_mix_answer')
        ...
        // voice line caused "I'm 秒答… I don't have a product" in real
