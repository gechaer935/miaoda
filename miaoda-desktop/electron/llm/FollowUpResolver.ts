...
// topic ("how is IT developed?" → project "秒答"). This resolver covers the
 ...
 * prior context. NEVER says "I'm 秒答 / an AI assistant", never dumps profile,
...
function prevWasProject(ctx: FollowUpContext): boolean {
  return ctx.previousAnswerType === 'project_answer' || ctx.previousAnswerType === 'project_followup_answer'
    || !!ctx.lastEntity || /\bproject|built|developed|miaoda\b/.test(lc(ctx.previousQuestion));
}
