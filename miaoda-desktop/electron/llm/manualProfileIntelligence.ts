...
// GENUINE assistant-meta questions — these legitimately address 秒答 (the
...
// manual-chat log showed them leaking "I'm 秒答, an AI assistant"). Only
// explicit AI/bot/model/who-built-you/what-is-秒答 asks remain assistant-meta.
...
const ASSISTANT_IDENTITY_PATTERNS = [
  ...
  new RegExp(`^${FILLER}what\\s+(is|s)\\s+miaoda\\b`),
  /\bwhat\s+(is|s)\s+this\s+(app|tool|product|assistant)\b/,
  new RegExp(`^${FILLER}who\\s+(made|built|created|developed|trained|designed)\\s+(you|this|miaoda|the app)\\b`),
  /\bwhat\s+(ai\s+)?model\s+(are\s+you|do\s+you\s+(use|run))\b|\bwhich\s+(llm|model)\b/,
  /\bare\s+you\s+(chatgpt|gpt|claude|gemini|miaoda)\b/,
];
...
const NAME_PATTERNS = [
  ...
  // 秒答, an AI assistant" / a false refusal.
...
// leaking "I'm 秒答" / refusing). Distinct from a bare NAME ask.
const INTRO_PATTERNS = [
  ...
  // drop it to the LLM (which leaked "I'm 秒答").
...
// "I'm 秒答" / refusing). Returns '' when the name is missing.
//
...
const findProjectByName = (profile: MaybeStructured<StructuredProfileFacts>, q: string): ProfileProject | null => {
  ...
  // question. Project names are often "秒答 – Open Source AI Meeting Copilot"
  // while the question just says "miaoda", so match on the FIRST significant
...
// "My project 秒答 is A privacy-first..." (real manual log 2026-06-12).
...
export const tryBuildManualProfileFastPathAnswer = ({
  ...
  // PERSON AS the candidate ("I'm 秒答, …" / "My name is …"), NOT in second
  // person ("Your name is …") and NOT as the assistant ("I'm 秒答, an AI
  ...
  // 秒答?", "who made you?") always bails to the assistant path (returns null),
  ...
  // fix (release 2026-06-08): "who are you?" in profile mode → "My name is 秒答",
  // never "Your name is…" or "I'm 秒答".
  const lc = qNorm;
  ...
  // reaching the LLM and answering "I'm 秒答, an AI assistant" — wrong when a
  ...
  // assistant-meta "who are you / what is 秒答" (those still bail above via
  ...
  // Phase 10: single-project FAST PATH — "tell me about 秒答", "best
  // project", "tech stack of 秒答". Deterministic from the matched project
  ...
  // NOTE: this branch does NOT gate on `qualified` — "tell me ABOUT 秒答"
