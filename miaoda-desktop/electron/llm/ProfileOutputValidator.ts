...
//      秒答" / "I'm an AI assistant" when the interviewer asked the CANDIDATE.
...
export type ProfileViolationCode =
  ...
  // FORBIDDEN) that leaked "秒答", the candidate name, a loaded project/company
  // name, or profile/JD/salary references. Flash-lite intermittently appends a
  // stray "秒答" mention to clean coding answers; this is the deterministic
...
export interface ProfileValidationInput {
   ...
   * "秒答"/profile-marker check runs (release 2026-06-07).
   */
  profileTokens?: {
    firstName?: string;
    projects?: string[];
   ...
   * answer ("use my 秒答 project as an example", "how did you implement this in
   * 秒答?"). Suppresses the coding-leak check so an intentional reference is
...
// "I am 秒答" / "I'm an AI assistant" — the assistant identity leaking into a
...
const ASSISTANT_IDENTITY_RE =
  /\bI(?:'m| am)\s+秒答|\bI(?:'m| am)\s+an?\s+(?:AI\s+)?(?:assistant|language model|chat\s?bot)\b|\bI(?:'m| am)\s+an\s+AI\b(?!\s*(?:and|engineer|developer|intern|specialist|enthusiast)\b)(?![\s]*[&/,])|\bas\s+an\s+AI(?:\s+(?:language\s+)?model)?,?\s+I\b/i;
const MIAODA_SELF_RE = /\b(?:I am|I'm|as)\s+秒答/i;
...
// (秒答), the candidate, a loaded project/company, or reference the
...
// The PRODUCT name in either spelling. "秒答" is unambiguous, so it always counts.
// The ASCII "miaoda" only counts when preceded by a reference cue (in/the/a/using/
// built/from/my/your miaoda) — a bare lowercase mention can be incidental prose and
// is NOT a leak (release 2026-06-07: "X ... supports Y" phrasing in coding answers
// was a false positive). NOTE: `\b` never holds beside a CJK character, so "秒答"
// must be matched WITHOUT word boundaries or the branch is dead.
const PRODUCT_MIAODA_RE = /\b(?:[Ii]n|[Tt]he|[Aa]|[Aa]n|[Uu]sing|[Uu]sed?|[Bb]uilt?|[Ff]rom|[Vv]ia|[Ww]ith|[Mm]y|[Yy]our)\s+miaoda\b|秒答/;
...
const codingProfileMarkerHit = (s: string): boolean => PRODUCT_MIAODA_RE.test(s) || PROFILE_MARKER_NON_PRODUCT_RE.test(s);
// The user explicitly invited the project/profile into a technical answer.
const PROFILE_INVITE_RE = /\b(use|using|with|in|from)\s+(my|your|the)\s+(miaoda|project|portfolio|own (project|code))\b|\bhow (did|do) you (implement|build|use)\s+(this|that|it)\s+in\s+(miaoda|your project)\b|\bin miaoda\b|\b(my|your) miaoda project\b|\bas an example from\b/i;
...
function thirdPersonAboutUser(answer: string): boolean {
  ...
  return /\b(the user'?s?|the candidate'?s?|their\s+(?:name|experience|background|projects?|skills?))\b/i.test(answer)
    ...
    || /\byou\s+are\s+[A-Z][a-z]+/i.test(answer); // "You are 秒答 ..."
}
...
export function validateProfileOutput(input: ProfileValidationInput): ProfileValidationResult {
  ...
  // 秒答" is fine, so gate on candidateDirected + profile answer type.
  if (isProfile && candidateDirected && (ASSISTANT_IDENTITY_RE.test(text) || MIAODA_SELF_RE.test(text))) {
    violations.push({
      code: 'assistant_identity_leak',
      detail: `${plan.answerType} answered as the assistant ("I am 秒答 / an AI") instead of the candidate`,
      severity: 'error',
    });
  }
  ...
  // must not name the product (秒答), the candidate, a loaded project/company,
  ...
  if (PROFILE_FORBIDDEN_OUTPUT_TYPES.has(plan.answerType) && !input.profileExplicitlyInvited) {
    ...
    //    `秒答` is still a reference/leak, just styled as code.
    ...
    // Both drop FENCED code blocks but KEEP code COMMENTS ("-- as used in 秒答"),
    ...
    const tokenHit = dynamicTokens.find(tok => {
      // "秒答" the product is handled case-sensitively by PRODUCT_MIAODA_RE
      // below — skip it here so the dynamic (case-insensitive) check doesn't match
      // the English adverb "miaoda".
    ...
    // `秒答` reference counts). Comp words: test WITHOUT inline code (a `salary`
    // identifier doesn't).
    // Proper-noun / profile-reference markers (product name is case-sensitive via
    // PRODUCT_MIAODA_RE so the adverb "miaoda" is not a false leak): test on
    // prose WITH inline code (a `秒答` reference counts).
    ...
    if (PRODUCT_MIAODA_RE.test(prose) || NAME_MARKER_RE.test(prose) || COMP_MARKER_RE.test(proseNoInlineCode) || tokenHit) {
      violations.push({
 ...
 * blocks verbatim (a stray "秒答" almost always lands in prose, not code).
...
export function stripProfileTokensFromCoding(answer: string, tokens: string[]): string {
  ...
  const markers = [PRODUCT_MIAODA_RE, /\b(my|your) (resume|profile|cv|JD)\b/i,
    ...
    // "秒答" is handled case-sensitively by PRODUCT_MIAODA_RE above; a token
  ...
  const cleaned = parts.map(seg => {
    if (seg.startsWith('```')) {
      ...
      // inside the block ("-- as used in 秒答", "// from my resume"). Scrub
...
// assistant-meta — "as an AI assistant", "I'm 秒答", "I can't share", "I don't
...
// ("I'm 秒答, an AI assistant" / "I was") or a stock
...
// misfire ("I'm an AI assistant." / "I'm an AI assistant")
...
// MEDIUM-1.
const ASSISTANT_IDENTITY_MISFIRE_RE = /\bI(?:'m| am)\s+秒答|\bI(?:'m| am)\s+an?\s+(?:AI\s+)?(?:assistant|language model|chat\s?bot)(?=\s*(?:[.,!?;]|$|\s+(?:developed|created|made|built|designed|trained|here|created|that|who|which|to\b|and\s+I\b)))|\bI\s+was\s+(?:developed|made|built|created)\s+by\s+(?:the\s+)?秒答|\bas\s+an\s+AI(?:\s+(?:language\s+)?model)?,?\s+I\b/i;
  ...
  for (const code of new Set(result.errorCodes)) {
    switch (code) {
      ...
      case 'assistant_identity_leak':
        lines.push('- Answer AS the candidate in first person ("My name is ...", "I worked on ..."). Never say you are 秒答 or an AI.');
        break;
      ...
      case 'profile_token_in_coding_answer':
        lines.push('- This is a PURE technical/coding answer. Do NOT mention 秒答, the candidate, any project or company name, the resume/profile/JD, or salary. Answer the algorithm/concept only, from general knowledge.');
        break;
    }
  }
  return lines.length
