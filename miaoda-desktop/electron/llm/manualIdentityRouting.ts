...
// and "introduce yourself" with the canned "I'm 秒答, an AI assistant."
...
//     秒答? what is 秒答?) → canned assistant reply, always.
...
const ASSISTANT_META_PROBE_RE =
  /^\s*(?:so|wait|ok(?:ay)?|um|hey|but|and|actually)?[\s,]*(what\s+(are|r)\s+(you|u)|are\s+you\s+(chatgpt|gpt[-\s]?\d?|claude|gemini|llama|an?\s+(ai|bot|llm|model|assistant)|human|real|a\s+robot)|what('?s|\s+is)\s+your\s+model|which\s+(ai|model|llm)\s+are\s+you|who\s+(made|built|created|developed|trained)\s+(you|this|miaoda|秒答)|what\s+model\s+(are\s+you|do\s+you\s+use)|what\s+(is|'?s)\s+(miaoda|秒答))\s*\??\s*$/i;

/** Creator probes — always the assistant's creator. */
const CREATOR_PROBE_RE =
  /^\s*(who\s+(made|built|created|developed|trained)\s+(you|this|miaoda|秒答))\s*\??\s*$/i;
...
export function resolveIdentityProbe(message: string, profileReady: boolean): IdentityProbeDecision {
  ...
  if (CREATOR_PROBE_RE.test(message)) {
    return { kind: 'assistant_reply', reply: 'I was made by the 秒答 team.' };
  }
  if (ASSISTANT_META_PROBE_RE.test(message)) {
    return { kind: 'assistant_reply', reply: "I'm 秒答, an AI assistant." };
  }
  if (CANDIDATE_AMBIGUOUS_PROBE_RE.test(message)) {
    ...
    return profileReady
      ...
      : { kind: 'assistant_reply', reply: "I'm 秒答, an AI assistant." };
  }
  return { kind: 'none' };
}
