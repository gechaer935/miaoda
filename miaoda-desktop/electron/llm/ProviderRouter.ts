export type LLMProviderId = 'groq' | 'codex' | 'gemini_flash' | 'gemini_pro' | 'openai' | 'claude' | 'deepseek' | 'zhipu' | 'ollama';
...
export function routeLLMProviders(options: ProviderRouteOptions): ProviderAttempt[] {
    ...
    const zhipu: ProviderSpec = {
        provider: 'zhipu',
        name: `Zhipu GLM (${models.zhipu ?? 'default'})`,
        model: models.zhipu,
        available: Boolean(availability.hasZhipu),
        unavailableReason: 'missing_api_key',
        supports: ['chat', 'stream_chat', 'structured'],
    };
    ...
    const orderedSpecs: ProviderSpec[] = options.multimodal
        ...
        : [groq, codex, zhipu, geminiFlash, geminiPro, openai, claude, deepseek];
...
export class ProviderRouter {
    ...
    constructor(circuitConfig?: Partial<CircuitBreakerConfig>) {
        ...
        ['gemini', 'groq', 'openai', 'claude', 'deepseek', 'zhipu', 'codex'].forEach(provider => {
    ...
    selectProvider(policy: RoutingPolicy): ProviderChoice {
        ...
        const availableProviders = this.filterHealthyProviders(
            ['gemini', 'groq', 'openai', 'claude', 'deepseek', 'zhipu', 'codex'],
            health
        );
