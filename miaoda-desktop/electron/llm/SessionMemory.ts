...
// FollowUpResolver lacks: when an interviewer mentions "秒答" at minute 1 and at
// minute 62 asks "what was the hardest part of that project?", we must resolve
// "that project" → 秒答 even though it's far outside the transcript window.
//
...
export type MemoryItemKind =
  | 'project'     // a named project on the table ("秒答", "TalentScope")
...
export interface MemoryItem {
  kind: MemoryItemKind;
  /** Short token/phrase (e.g. "秒答", "Python", "price too high"). Never raw PII. */
  value: string;
...
export interface MemoryQuery {
   ...
   *  used this in 秒答?" during coding) — allows the otherwise-blocked recall. */
