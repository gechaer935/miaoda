...
// only when MIAODA_PI_TELEMETRY_DEBUG=true. A sink can be registered (e.g. to ship
...
class PiTelemetry {
  ...
  emit(event: PiTelemetryEvent, data: Record<string, unknown> = {}): void {
    ...
    let debug = false;
    try { debug = (process.env.MIAODA_PI_TELEMETRY_DEBUG || '').trim().toLowerCase() === 'true'; } catch { /* ignore */ }
    if (debug) {
