...
function readEnvOverride(): 'on' | 'off' | null {
  ...
  try {
    const v = (process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY || '').trim().toLowerCase();
...
function killSwitchEngaged(): boolean {
  try {
    const v = (process.env.MIAODA_LIVE_SESSION_MEMORY_KILL_SWITCH || '').trim().toLowerCase();
...
function isInternalContext(): boolean {
  try {
    ...
    if (process.env.MIAODA_INTERNAL === '1' || process.env.MIAODA_DEV === '1') return true;
  } catch { /* default false */ }
  return false;
}
...
function rolloutPercent(): number | null {
  try {
    const raw = process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT;
...
export function liveSessionMemoryMaxItems(): number {
  try {
    const v = parseInt(process.env.MIAODA_SESSION_MEMORY_MAX_ITEMS || '', 10);
...
export function liveSessionMemoryDebug(): boolean {
  try { return (process.env.MIAODA_SESSION_MEMORY_DEBUG || '').trim().toLowerCase() === 'true'; }
  catch { return false; }
}
