 ...
 * candidate answer ("秒答.") as a project name. Returns [] for filler/noise.
 */
export function extractTranscriptEntities(text: string, speakerRole?: 'interviewer' | 'user' | 'assistant'): ExtractedEntity[] {
  ...
  // a SHORT candidate answer that is just a proper noun ("秒答.") names a project.
...
export function isExplicitCrossModeInvite(text: string): boolean {
  const t = String(text || '');
  return /\b(use|using|with|in|from)\s+(my|your|this|that|the)\s+(miaoda|project|portfolio|own (project|code|app))\b/i.test(t)
    || /\bin miaoda\b/i.test(t)
    || /\bhave you (?:used|done|built|implemented|applied)\b[^.?!]*\bin\s+(your|my|this|that|the)\s+(miaoda|project|portfolio|app|product|work|experience)\b/i.test(t);
}
