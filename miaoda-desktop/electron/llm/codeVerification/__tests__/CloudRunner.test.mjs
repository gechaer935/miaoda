...
describe('cloudRunner — gated OFF by default', () => {
  test('cloudExecutionEnabled() is false without the opt-in flag', () => {
    const prev = process.env.MIAODA_CODE_EXECUTION_CLOUD;
    delete process.env.MIAODA_CODE_EXECUTION_CLOUD;
    try {
      assert.equal(cloudExecutionEnabled(), false);
    } finally {
      if (prev !== undefined) process.env.MIAODA_CODE_EXECUTION_CLOUD = prev;
    }
  });
  ...
  test('even with the flag on, the stub does not execute (pending) — never a false pass', async () => {
    const prev = process.env.MIAODA_CODE_EXECUTION_CLOUD;
    process.env.MIAODA_CODE_EXECUTION_CLOUD = 'true';
    try {
      const r = await runCaseCloud('java', 'class Solution {}', 'f', tc);
      ...
      if (prev !== undefined) process.env.MIAODA_CODE_EXECUTION_CLOUD = prev; else delete process.env.MIAODA_CODE_EXECUTION_CLOUD;
    }
  });

  test('pistonUrl honors the override env, falls back to default', () => {
    const prev = process.env.MIAODA_PISTON_URL;
    process.env.MIAODA_PISTON_URL = 'https://piston.internal/api';
    try {
      assert.equal(pistonUrl(), 'https://piston.internal/api');
    } finally {
      if (prev !== undefined) process.env.MIAODA_PISTON_URL = prev; else delete process.env.MIAODA_PISTON_URL;
    }
    delete process.env.MIAODA_PISTON_URL;
