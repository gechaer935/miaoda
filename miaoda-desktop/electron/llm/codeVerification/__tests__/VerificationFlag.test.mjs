...
// (no redeploy) via env MIAODA_CODE_VERIFY=off. When off, the hidden
...
describe('isCodeVerificationEnabled', () => {
  ...
  for (const off of ['off', 'false', '0', 'disabled']) {
    test(`env MIAODA_CODE_VERIFY=${off} disables it (child process for clean cache)`, () => {
      ...
      ], { cwd: process.cwd(), env: { ...process.env, MIAODA_CODE_VERIFY: off } }).toString();
  ...
  test('env=on (or unset) keeps it enabled', () => {
    const out = execFileSync(process.execPath, [
      '--input-type=module', '-e',
      `import { isCodeVerificationEnabled } from './dist-electron/electron/llm/codeVerification/verificationEnabled.js'; process.stdout.write(String(isCodeVerificationEnabled()));`,
    ], { cwd: process.cwd(), env: { ...process.env, MIAODA_CODE_VERIFY: 'on' } }).toString();
