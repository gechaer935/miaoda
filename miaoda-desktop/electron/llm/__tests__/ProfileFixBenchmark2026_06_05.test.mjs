...
describe('fix: JD-fit routing', () => {
  for (const q of [
    ...
    '秒答 is not data analysis, so why is it relevant?',
...
describe('fix: single-project fast path (deterministic, zero latency)', () => {
  const profile = {
    ...
    projects: [
      { name: '秒答 – Open Source AI Meeting Copilot', description: 'a privacy-first AI meeting assistant', technologies: ['Electron', 'TypeScript', 'Rust'] },
  ...
  test('"Tell me about 秒答." → deterministic answer naming the project + stack', () => {
    const r = fp('Tell me about 秒答.');
    assert.ok(r && r.answer, 'expected a fast-path answer');
    assert.match(r.answer, /秒答/);
    assert.match(r.answer, /Electron|TypeScript|Rust/);
  });
  test('"What tech stack did you use in 秒答?" → deterministic stack answer', () => {
    const r = fp('What tech stack did you use in 秒答?');
    assert.ok(r && r.answer, 'expected a fast-path answer');
    assert.match(r.answer, /Electron|TypeScript|Rust/);
  });
  test('narrative drill-in "What was your role in 秒答?" defers to LLM (no fast path)', () => {
    const r = fp('What was your role in 秒答?');
    assert.equal(r, null, 'narrative drill-in should reach the grounded LLM');
  });
  test('"how was 秒答 developed?" defers to LLM (narrative)', () => {
    const r = fp('how was 秒答 developed?');
    assert.equal(r, null);
  });
});
