...
//   "who are you?" / "what is your name?" → "I'm 秒答, an AI assistant." (WRONG)
...
// A loaded candidate profile (synthetic — never 秒答-specific hardcoding).
const PROFILE = {
...
describe('Manual identity — candidate first-person, never "I\'m 秒答"', () => {
  for (const q of ['who are you?', 'what is your name?', 'what should I call you?']) {
    test(`"${q}" → first-person candidate name, no assistant identity`, () => {
      ...
      assert.doesNotMatch(r.answer, /I'?m 秒答|AI assistant|Your name is/i);
    });
  }
  test('"introduce yourself" → first-person intro', () => {
    ...
    assert.doesNotMatch(r.answer, /I'?m 秒答|AI assistant/i);
  });
...
describe('Manual assistant-meta — answers about the app, NOT the candidate', () => {
  for (const q of ['are you an AI?', 'what is 秒答?', 'who made 秒答?', 'what model are you?']) {
