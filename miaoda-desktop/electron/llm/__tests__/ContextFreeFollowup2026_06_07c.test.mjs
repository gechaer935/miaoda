...
// mode-appropriate clarification (never "I'm 秒答"/profile dump/false refusal)
...
const IDENTITY_LEAK = /\bI(?:'m| am)\s+秒答|\bAI assistant\b|\bas an AI\b/i;
...
describe('isBareFollowUp — shape detection', () => {
  ...
  for (const q of ['why should we hire you?', 'tell me about 秒答', 'what is your name', 'solve two sum', 'rate your python out of 10', 'how do you handle pressure']) {
...
describe('resolveFollowUpOrClarify — resolve normally WHEN prior context exists', () => {
  test('"why?" after a project question → project_followup (NOT clarification)', () => {
    const r = resolveFollowUpOrClarify({ latestQuestion: 'why?', previousQuestion: 'tell me about your best project', lastEntity: '秒答', previousAnswerType: 'project_answer', surface: 'what_to_answer' });
    assert.notEqual(r.isClarification, true);
