...
describe('extractProjectEntity edge cases', () => {
  test('explicit capitalized name', () => {
    assert.equal(extractProjectEntity('how is 秒答 developed?'), '秒答');
  });
  ...
  test('stopword-prefixed REAL name keeps the name ("The 秒答" → "秒答")', () => {
    ...
    assert.equal(extractProjectEntity('how is The 秒答 built?'), '秒答');
  });
});
...
describe('project_followup pronoun path + followUpTarget resolution', () => {
  ...
  test('pronoun + followUpTarget from prior turn resolves the entity', () => {
    const r = p({ q: 'how was it built?', eq: { latestQuestion: 'how was it built?', questionType: 'follow_up', followUpTarget: '秒答', confidence: 0.8, detectedSpeaker: 'interviewer', isFollowUp: true } });
    assert.equal(r.answerType, 'project_followup_answer');
