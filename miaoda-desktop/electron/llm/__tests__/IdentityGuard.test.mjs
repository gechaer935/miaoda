...
// modes (model outputs "I'm 秒答..." as the speaker's self-introduction).
//
...
describe('IdentityGuard — TINY_CORE carries the name-claim guard', () => {
  ...
  test('TINY_CORE explicitly forbids "I\'m 秒答" / "I\'m 秒答"', () => {
    assert.ok(
      /I'm 秒答/i.test(tiny.TINY_CORE),
      'TINY_CORE must call out the literal "I\'m 秒答" anti-pattern',
    );
    assert.ok(
      /I'm 秒答/i.test(tiny.TINY_CORE),
      'TINY_CORE must call out the literal "I\'m 秒答" anti-pattern',
...
describe('IdentityGuard — full-tier prompts.ts retains the rule', () => {
  test('UNIVERSAL_WHAT_TO_ANSWER_PROMPT contains the name-confusion rule', () => {
    assert.ok(
      /names "秒答" and "秒答" describe ONLY this assistant/.test(
  ...
  test('CUSTOM_ANSWER_PROMPT keeps the creator-question response phrasing', () => {
    assert.ok(
      /"I was\."/.test(full.CUSTOM_ANSWER_PROMPT),
