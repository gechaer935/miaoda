...
describe('P2: identity probe routing', () => {
    ...
    test('candidate-ambiguous probes stay canned with NO profile', () => {
        for (const q of ['who are you?', 'introduce yourself']) {
            ...
            assert.match(d.reply, /I'm 秒答/);
        }
    });
    test('assistant-meta probes stay canned even WITH a profile', () => {
        const cases = [
            ['are you an AI?', /I'm 秒答/],
            ['are you chatgpt?', /I'm 秒答/],
            ['what is miaoda?', /I'm 秒答/],
            ['what model are you?', /I'm 秒答/],
            ['who built miaoda?', /秒答 team/],
            ['who made you', /秒答 team/],
        ];
    ...
    test('the fast path actually answers the routed probes (no 秒答 leak possible)', () => {
        for (const q of ['who are you?', 'what is your name?', 'introduce yourself']) {
            ...
            assert.doesNotMatch(r.answer, /秒答|AI assistant/i);
        }
    });
});
...
describe('P6: sales template voice', () => {
    ...
    test('sales questions carry seller voice + assistant-identity ban + resume ban', () => {
        for (const q of salesQs) {
            ...
            assert.match(c, /NEVER say "I'm 秒答"|Never identify as an AI assistant/, q);
...
describe('P7: project tech-stack/source routing', () => {
    ...
    test('"what is 秒答 built with?" → project_about (tech stack), never source-evidence refusal', () => {
        assert.equal(route('what is 秒答 built with?'), 'project_about_answer');
    });
    test('"is 秒答 open source?" → project_about', () => {
        assert.equal(route('is 秒答 open source?'), 'project_about_answer');
    });
    test('"show me the exact source code" → source_code_evidence (refuse unless loaded)', () => {
        assert.equal(route('show me the exact source code of 秒答'), 'source_code_evidence_answer');
    });
    test('"give me the GitHub link" → project_link (loaded link only)', () => {
        assert.equal(route('give me the GitHub link for 秒答'), 'project_link_answer');
    });
});
