...
describe('ProfileOutputValidator: clean answers pass', () => {
  test('correct first-person identity answer passes', () => {
    const r = validate('My name is 秒答.', 'What is your name?');
    assert.equal(r.ok, true, JSON.stringify(r.violations));
  });
  test('correct first-person experience answer passes', () => {
    const r = validate("I worked on 秒答, an AI meeting assistant, and interned building WebRTC pipelines.", 'Tell me about your experience.');
    assert.equal(r.ok, true, JSON.stringify(r.violations));
  });
...
describe('ProfileOutputValidator: catches the spec §7 failure modes', () => {
  test('1. assistant-identity leak ("I am 秒答") on an identity question', () => {
    const r = validate("I am 秒答, an AI assistant.", 'What is your name?');
  ...
  test('5. third-person about the user instead of first person', () => {
    const r = validate("The user's name is 秒答 and their experience includes 秒答.", 'What is your name?');
    assert.equal(r.ok, false);
...
describe('ProfileOutputValidator: repair instructions', () => {
  ...
  test('identity leak yields a first-person correction', () => {
    ...
    assert.match(instr, /Never say you are 秒答/i);
  });
...
describe('ProfileOutputValidator: does not over-flag legitimate answers', () => {
  test('candidate saying "I" many times is fine', () => {
    const r = validate("I'm a software engineer. I built 秒答 and I led the WebRTC work.", 'Tell me about yourself.');
    assert.equal(r.ok, true, JSON.stringify(r.violations));
  });
