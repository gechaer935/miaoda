...
// mutating them here would cause cross-file flakes. We use MIAODA_INTERNAL/_DEV
// (read by nothing else) to exercise the internal-context tier.
const ENV_KEYS = [
  'MIAODA_ENABLE_LIVE_SESSION_MEMORY', 'MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT',
  'MIAODA_LIVE_SESSION_MEMORY_KILL_SWITCH', 'MIAODA_INTERNAL', 'MIAODA_DEV',
];
let saved = {};
...
describe('Rollout — production default ON (PI v3 W6d), overrides still win', () => {
  ...
  // Use MIAODA_INTERNAL (a flag no other concurrent test file reads) to exercise the
  ...
  test('MIAODA_INTERNAL=1 → ON (internal_context)', () => {
    process.env.MIAODA_INTERNAL = '1'; __resetLiveSessionMemoryCache();
    const c = resolveLiveSessionMemoryConfig('s');
    assert.equal(c.enabled, true);
    assert.equal(c.reason, 'internal_context');
  });
  test('MIAODA_DEV=1 → ON', () => {
    process.env.MIAODA_DEV = '1'; __resetLiveSessionMemoryCache();
...
describe('Rollout — KILL SWITCH overrides everything', () => {
  test('kill switch disables even when env ON', () => {
    process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY = 'on';
    process.env.MIAODA_LIVE_SESSION_MEMORY_KILL_SWITCH = 'true'; __resetLiveSessionMemoryCache();
    const c = resolveLiveSessionMemoryConfig('s');
  ...
  test('kill switch disables even in an internal context', () => {
    process.env.MIAODA_INTERNAL = '1';
    process.env.MIAODA_LIVE_SESSION_MEMORY_KILL_SWITCH = '1'; __resetLiveSessionMemoryCache();
...
describe('Rollout — percentage gate (production gradual rollout)', () => {
  test('percent 0 → OFF', () => {
    process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '0'; __resetLiveSessionMemoryCache();
    const c = resolveLiveSessionMemoryConfig('s'); assert.equal(c.enabled, false); assert.equal(c.reason, 'rollout_out');
  });
  test('percent 100 → ON', () => {
    process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '100'; __resetLiveSessionMemoryCache();
  ...
  test('percent 50 → roughly half of sessions in, deterministically', () => {
    process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '50'; __resetLiveSessionMemoryCache();
  ...
  test('deterministic bucketing: same session id is stable across calls', () => {
    process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '37'; __resetLiveSessionMemoryCache();
  ...
  test('partial percent with NO sessionId → OFF (test-engineer gap: id-less sessions not lumped in)', () => {
    process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '90'; __resetLiveSessionMemoryCache();
...
describe('Rollout — flag OFF/ON helper parity', () => {
  test('isLiveSessionMemoryEnabled mirrors resolveLiveSessionMemoryConfig.enabled', () => {
    process.env.MIAODA_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '63'; __resetLiveSessionMemoryCache();
...
describe('Telemetry — marker only, no sensitive content', () => {
  ...
  test('scrubTelemetry is an ALLOWLIST — keeps markers, drops everything else', () => {
    const out = scrubTelemetry({
      ...
      recalledValue: '秒答', entityValue: '秒答',
    });
  ...
  test('an allowed key with a free-text / number value is still dropped (value backstop)', () => {
    const out = scrubTelemetry({ reason: 'recalled the 秒答 project from minute 1 of the conversation', mode: 'sales' });
  ...
  test('emit stores a scrubbed record', () => {
    piTelemetry.emit('session_memory_recall_succeeded', { recalledKind: 'project', ageBucket: '60min+', recalledValue: '秒答' });
