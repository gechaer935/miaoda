...
const TOKENS = { firstName: '秒答', projects: ['秒答', 'TalentScope'], companies: ['EstroTech'] };

describe('Pattern 1: "what is 秒答 built with" → product-about', () => {
  for (const q of [
    'what is natively built with', 'what tech stack is 秒答 built with',
    'what is 秒答 made using', 'what are the technologies behind 秒答',
    'what is the architecture of 秒答',
  ]) {
  ...
  test('"how did you build 秒答" → a project-grounded answer (about-alias)', () => {
    assert.ok(['project_about_answer', 'project_answer', 'project_followup_answer'].includes(plan('how did you build 秒答').answerType));
  });
});
...
describe('Pattern 3+4: coding-answer profile-token leak validator', () => {
  test('stray "秒答" in a DSA answer is flagged', () => {
    const leak = '## Approach\nUse a min-heap.\n```python\ndef kth(a,k): return sorted(a)[-k]\n```\nI used this exact approach in 秒答.';
  ...
  test('strip removes the offending prose sentence, preserves code', () => {
    const leak = '## Approach\nUse a min-heap.\n```python\ndef kth(a,k): return sorted(a)[-k]\n```\nI used this exact approach in 秒答 to rank results.';
    const stripped = stripProfileTokensFromCoding(leak, ['秒答', '秒答', 'TalentScope', 'EstroTech']);
  ...
  test('EXPLICIT invite ("use my 秒答 project") suppresses the leak flag', () => {
    const invited = '## Approach\n```py\nx=1\n```\nHere is how I did it in 秒答.';
...
describe('Forbidden-answer leak strip covers non-coding types (sales/meeting)', () => {
  ...
  test('"I\'m 秒答, an AI assistant…" identity leak in a SALES answer is flagged', () => {
    const leak = "I'm 秒答, an AI assistant. I don't have pricing info.";
  ...
  test('the identity-leak preamble strips out of a meeting answer', () => {
    const leak = "I'm 秒答, an AI assistant. The next step is owned by Sarah.";
    const s = stripProfileTokensFromCoding(leak, ['秒答', '秒答']);
    assert.doesNotMatch(s, /natively|AI assistant|秒答/i);
...
describe('Code-review hardening: strip preserves code fences + collision tokens', () => {
  ...
  test('the English adverb "natively" is NOT flagged; the product "秒答" IS', () => {
    for (const clean of [
    ...
    for (const leak of [
      'I built this exact pattern in 秒答.',
      'The `秒答` ranker does this.',
  ...
  test('a SQL `salary` COLUMN is not a leak, but a `秒答` inline reference is', () => {
    ...
    const inlineName = 'Use a heap. The `秒答` ranker does exactly this.';
  ...
  test('strip removes a full identity-leak preamble from a profile-forbidden (meeting) answer', () => {
    const leak = "I'm 秒答, an AI assistant. I was made by the 秒答 team. The next step is owned by Sarah.";
    const s = stripProfileTokensFromCoding(leak, ['秒答', '秒答']);
    assert.doesNotMatch(s, /natively|AI assistant|秒答/i);
    assert.match(s, /owned by Sarah/);
  });
...
describe('Product "秒答" vs adverb "natively" (planner sanity)', () => {
  ...
  test('"how is 秒答 built" still routes to a product answer', () => {
    assert.ok(['project_about_answer', 'project_answer', 'project_followup_answer'].includes(plan('how is 秒答 built').answerType));
  });
