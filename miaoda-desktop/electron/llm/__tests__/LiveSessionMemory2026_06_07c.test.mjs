...
describe('Feature flag — env override wins both directions', () => {
  test('MIAODA_ENABLE_LIVE_SESSION_MEMORY=off forces OFF even in a test/benchmark context', () => {
    const prev = process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY;
    process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY = 'off'; __resetLiveSessionMemoryCache();
    assert.equal(isLiveSessionMemoryEnabled(), false);
    process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY = prev ?? ''; __resetLiveSessionMemoryCache();
  });
  test('MIAODA_ENABLE_LIVE_SESSION_MEMORY=on forces ON', () => {
    const prev = process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY;
    process.env.MIAODA_ENABLE_LIVE_SESSION_MEMORY = 'on'; __resetLiveSessionMemoryCache();
...
describe('transcript entity extraction (independent — not from any answer key)', () => {
  test('"Tell me about 秒答." → project 秒答', () => {
    assert.ok(extractTranscriptEntities('Tell me about 秒答.', 'interviewer').some(e => e.kind === 'project' && e.value === '秒答'));
  });
  test('short candidate answer "秒答." → project', () => {
    assert.ok(extractTranscriptEntities('秒答.', 'user').some(e => e.kind === 'project' && e.value === '秒答'));
  });
  ...
  test('correction cue detected', () => {
    ...
    assert.equal(isCorrectionTurn('My best project is 秒答.'), false);
  });
  test('explicit cross-mode invite detected', () => {
    assert.equal(isExplicitCrossModeInvite('have you used this in 秒答?'), true);
    assert.equal(isExplicitCrossModeInvite('solve two sum'), false);
  });
});

describe('resolveLiveFollowup — the 12 live edge cases (Phase 3)', () => {
  test('1. immediate project follow-up → project_followup, 秒答', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Tell me about 秒答.', t: 0 }, { role: 'user', text: 'An AI copilot.', t: 5 }, { role: 'interviewer', text: 'How did you build it?', t: 8 }], latestQuestion: 'How did you build it?', mode: 'technical-interview', surface: 'what_to_answer' });
    assert.equal(r.recalledEntity, '秒答');
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
  });
  test('2. delayed (8 filler turns) → "tech stack there?" resolves 秒答', () => {
    const turns = [{ role: 'interviewer', text: 'Tell me about 秒答.', t: 0 }, { role: 'user', text: 'An AI copilot.', t: 5 }];
  ...
  test('3. one-hour project follow-up → 秒答', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Tell me about 秒答.', t: 1 * MIN }, { role: 'user', text: 'An AI copilot.', t: 2 * MIN }, { role: 'interviewer', text: 'filler', t: 30 * MIN }, { role: 'interviewer', text: 'What was the hardest part of that project?', t: 62 * MIN }], latestQuestion: 'What was the hardest part of that project?', mode: 'technical-interview', surface: 'what_to_answer' });
    assert.equal(r.recalledEntity, '秒答');
    assert.match(r.resolvedQuestion, /秒答/);
  });
  ...
  test('6. correction → TalentScope wins', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'What is your best project?', t: 0 }, { role: 'user', text: '秒答.', t: 5 }, { role: 'user', text: 'Actually use TalentScope.', t: 60 }, { role: 'interviewer', text: 'Why is it your best?', t: 120 }], latestQuestion: 'Why is it your best?', mode: 'looking-for-work', surface: 'manual' });
    assert.equal(r.recalledEntity, 'TalentScope');
  });
  test('7. double correction A→B→A → 秒答', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Best project?', t: 0 }, { role: 'user', text: '秒答.', t: 5 }, { role: 'user', text: 'Actually use TalentScope.', t: 60 }, { role: 'user', text: 'Actually back to 秒答.', t: 120 }, { role: 'interviewer', text: 'Why is that your best?', t: 180 }], latestQuestion: 'Why is that your best?', mode: 'looking-for-work', surface: 'manual' });
    assert.equal(r.recalledEntity, '秒答');
  });
  test('8. cross-mode coding boundary → NO 秒答 recall', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Tell me about 秒答.', t: 0 }, { role: 'interviewer', text: 'Solve Two Sum.', t: 60 }], latestQuestion: 'Solve Two Sum.', mode: 'coding', surface: 'coding' });
  ...
  test('12. no-context bare "why?" → clarification (no identity leak)', () => {
    ...
    assert.doesNotMatch(r.clarificationText, /秒答|AI assistant/i);
  });
});
...
describe('ENGINE ADAPTER — ms timestamps converted to seconds (code-review unit-bug guard)', () => {
  ...
  test('62-minute project recall SURVIVES with ms→s conversion', () => {
    const turns = msTurns([
      ['interviewer', 'Tell me about 秒答.', 1],
    ...
    assert.equal(r.recalledEntity, '秒答', 'long-range recall must survive realistic ms timestamps');
  ...
  test('5-second-ago project recall works under ms conversion (short range)', () => {
    const turns = msTurns([
      ['interviewer', 'Tell me about 秒答.', 0],
...
describe('isContextFreeBareFollowup', () => {
  ...
  test('"why?" after an answerable question is NOT context-free', () => {
    assert.equal(isContextFreeBareFollowup('why?', [{ role: 'interviewer', text: 'Tell me about 秒答.', t: 0 }, { role: 'interviewer', text: 'why?', t: 60 }]), false);
  });
});
...
describe('effectiveMemoryMode — coding/comp intent overrides ambient mode (code-review HIGH fix)', () => {
  ...
  test('REAL coding-in-interview: project NOT recalled (the production gate, not a synthetic coding mode)', () => {
    ...
    const turns = [{ role: 'interviewer', text: 'Tell me about 秒答.', t: 0 }, { role: 'interviewer', text: 'Solve two sum.', t: 60 }];
