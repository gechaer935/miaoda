...
const META = /\bas an AI\b|\bI(?:'m| am)\s+(?:an?\s+)?(?:AI|秒答)\b|\bcan(?:not| ?not|'?t)\s+share\b|\bdo(?:n'?t| not)\s+have\s+(?:access|your resume|your profile)\b|\bdo(?:n'?t| not)\s+(?:have|assign)\s+(?:numerical\s+)?ratings?\b/i;
...
describe('sanitizeCandidateAnswer — strips meta tail, keeps valid content', () => {
  const cases = [
    ...
    ['tail "I\'m 秒答"', 'My strongest skill is backend engineering with Python and SQL. I am 秒答, an AI assistant.', /backend engineering/],
...
describe('sanitizeCandidateAnswer — does NOT damage legitimate content', () => {
  for (const clean of [
    ...
    'I provide the resume screening feature in my product, 秒答.',
...
describe('sanitizeCandidateAnswer — "AI assistant" self-reference stripped, product description kept', () => {
  ...
  for (const keep of [
    'I built an AI assistant product called 秒答 that screens resumes.',
...
describe('sanitizeCandidateAnswer — all-meta answer flags needsFallback', () => {
  for (const allMeta of [
    'I am 秒答, an AI assistant. I cannot share that information.',
