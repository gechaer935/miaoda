...
describe('matrix: projects + project follow-ups', () => {
  ...
  for (const q of ['How is 秒答 developed?', 'What was your role in 秒答?', 'What was the hardest part of that project?', 'What tech stack did you use?', 'Why did you build it?', 'What did you learn from that project?']) {
  ...
  test('explicit entity is resolved', () => {
    assert.equal(p('How is 秒答 developed?').resolvedEntity, '秒答');
  });
});
