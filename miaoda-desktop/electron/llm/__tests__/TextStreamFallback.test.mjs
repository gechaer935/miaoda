...
// aborts first. Models a slow-prefill provider like the 秒答 gateway when its
...
describe('runStreamingTextFallback — race semantics', () => {
  test('fastest provider that produces a token wins and streams through', async () => {
    const health = new Map();
    const miaoda = okProvider('miaoda', ['Hello', ' world']);
    const out = await collect(runStreamingTextFallback([miaoda], health, DEFAULT_TEXT_FALLBACK_CONFIG, fastHooks()));
    assert.deepEqual(out, ['Hello', ' world']);
    assert.equal(miaoda._calls, 1);
  });

  test('a pre-commit error on the primary silently falls over to the next provider', async () => {
    const health = new Map();
    const primary = throwBeforeFirst('miaoda', 'fetch failed');
  ...
  test('a stalled primary (no first token) fails over within the TTFT budget', async () => {
    const health = new Map();
    const stalled = neverFirst('miaoda');
  ...
  test('a per-provider ttftTimeoutMs override lets a slow-first-token provider commit (MiniMax fallback regression)', async () => {
    // The 秒答 gateway can land on MiniMax (first token 3.3-7.7s) after the
    ...
    // ttftTimeoutMs override (mirrors LLMHelper.ts miaoda text entry) must let it
    ...
    const slowMiaoda = slowFirst('miaoda', 120, ['minimax answer'], { ttftTimeoutMs: 400 });
    const fallback = okProvider('groq', ['SHOULD-NOT-APPEAR']);
    const cfg = { ...DEFAULT_TEXT_FALLBACK_CONFIG, ttftTimeoutMs: 60, maxAttempts: 1 };
    const out = await collect(runStreamingTextFallback([slowMiaoda, fallback], health, cfg, { log: () => {}, warn: () => {} }));
  ...
  test('WITHOUT the override, the same slow-first-token provider is aborted by the default budget (proves the gate is real)', async () => {
    ...
    const health = new Map();
    const slowMiaoda = slowFirst('miaoda', 120, ['minimax answer']); // no override
  ...
  test('a post-commit failure does NOT switch providers (no duplicate output)', async () => {
    const health = new Map();
    const committed = throwAfterFirst('miaoda', ['partial answer'], 'socket hangup');
    const fallback = okProvider('groq', ['SHOULD-NOT-APPEAR']);
  ...
  test('exhaustion (all providers fail pre-commit) throws', async () => {
    const health = new Map();
    const a = throwBeforeFirst('miaoda', 'fetch failed');
