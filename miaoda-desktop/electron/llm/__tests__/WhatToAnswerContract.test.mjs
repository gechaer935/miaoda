...
// must FLAG (so the live path repairs) a 秒答-identity leak, a false refusal
...
describe('Issue 2: ProfileOutputValidator flags WTA contract violations', () => {
  ...
  test('"I am 秒答, an AI assistant." → assistant_identity_leak', () => {
    assert.ok(errs('I am 秒答, an AI assistant.').includes('assistant_identity_leak'));
  });
  ...
  test('"You are 秒答." (second/third person) → wrong_perspective', () => {
    assert.ok(errs('Your name is 秒答 and your experience includes engineering.').includes('wrong_perspective_not_first_person'));
  });
  test('"My name is 秒答." (correct first-person) → no error', () => {
    assert.equal(errs('My name is 秒答.').length, 0);
  });
  test('a legitimate job title "I\'m an AI Engineer" is NOT flagged as a 秒答 leak', () => {
