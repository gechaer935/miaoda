...
describe('FollowUpResolver: expand inherits prior route', () => {
  ...
  test('"Can you expand?" after a project answer → project_followup', () => {
    const r = resolveFollowUp({ latestQuestion: 'Can you expand?', previousQuestion: 'Tell me about 秒答.', previousAnswerType: 'project_answer', lastEntity: '秒答' });
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
    assert.equal(r.resolvedEntity, '秒答');
  });
...
describe('Issue 4: project drill-in resolves to project_followup on the entity', () => {
  test('"How is it developed?" after a project (entity 秒答) → project_followup', () => {
    const r = resolveFollowUp({ latestQuestion: 'How is it developed?', previousQuestion: 'Which is your best project?', lastEntity: '秒答' });
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
    assert.equal(r.resolvedEntity, '秒答');
    assert.match(r.resolvedQuestion, /秒答/);
  });
  test('"That project?" → project_followup on the entity', () => {
    const r = resolveFollowUp({ latestQuestion: 'That project?', previousQuestion: 'Tell me about 秒答.', lastEntity: '秒答' });
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
  });
});
...
describe('FollowUpResolver: gate boundaries + safety', () => {
  ...
  test('entity propagation without previousAnswerType still resolves a project expand', () => {
    const r = resolveFollowUp({ latestQuestion: 'Can you expand?', previousQuestion: 'Tell me about 秒答.', lastEntity: '秒答' });
    assert.equal(r.resolvedEntity, '秒答');
  });
