  ...
  static async resolve(config: AppAPIConfig): Promise<IEmbeddingProvider> {
    ...
    if (config.geminiKey) {
      try {
        ...
        // Rollback lever: MIAODA_GEMINI_EMBED_MODEL / _DIMS env vars pin the model
        ...
        const envModel = process.env.MIAODA_GEMINI_EMBED_MODEL;
        const envDims = process.env.MIAODA_GEMINI_EMBED_DIMS ? Number(process.env.MIAODA_GEMINI_EMBED_DIMS) : undefined;
