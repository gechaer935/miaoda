...
export class ProcessingHelper {
  ...
  constructor(appState: AppState) {
    ...
    if (useOllama) {
      ...
      let zhipuApiKey = process.env.ZHIPU_API_KEY || process.env.BIGMODEL_API_KEY
      ...
      this.llmHelper = new LLMHelper(apiKey, false, undefined, undefined, groqApiKey, openaiApiKey, claudeApiKey, deepseekApiKey, zhipuApiKey)
    }
  }
  ...
  public loadStoredCredentials(): void {
    ...
    const zhipuKey = credManager.getZhipuApiKey();
    ...
    if (zhipuKey) {
      console.log("[ProcessingHelper] Loading stored Zhipu API Key from CredentialsManager");
      this.llmHelper.setZhipuApiKey(zhipuKey);
    }
    ...
    // 秒答轻量发行版的问答、识图和鉴权均由 Go 服务处理，不启动旧版
    // Intelligence/RAG/模型版本后台任务。保留显式开关用于旧功能回归。
    if (process.env.NODE_ENV !== 'development' || process.env.MIAODA_LEGACY_FEATURES !== '1') return;
