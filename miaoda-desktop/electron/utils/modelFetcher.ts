...
type Provider = 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'zhipu';
...
const ZHIPU_DEFAULT_MODELS: ProviderModel[] = [
...
async function fetchZhipuModels(apiKey: string): Promise<ProviderModel[]> {
    try {
        ...
        const models: any[] = response.data?.data || [];
        if (!Array.isArray(models) || models.length === 0) {
            return ZHIPU_DEFAULT_MODELS;
        }

        const filtered = models.filter((m: any) => {
            const id = (m.id || '').toLowerCase();
            if (!id.startsWith('glm-')) return false;
            return !['asr', 'tts', 'voice', 'realtime', 'embedding', 'image', 'video'].some(p => id.includes(p));
        });

        if (filtered.length === 0) return ZHIPU_DEFAULT_MODELS;
        ...
        const status = error?.response?.status;
        if (status === 401 || status === 403) {
            throw new Error('Invalid or unauthorized Zhipu API key');
        }
        return ZHIPU_DEFAULT_MODELS;
    }
}
