...
use crate::audio_config::VAD_PREROLL_CHUNKS;
...
pub struct SilenceSuppressionConfig {
    ...
    /// Number of pre-speech frames to prepend when the gate opens.
...
impl Default for SilenceSuppressionConfig {
    fn default() -> Self {
        Self {
            ...
            preroll_chunks: VAD_PREROLL_CHUNKS,
        }
    }
}
...
pub struct SilenceSuppressor {
    ...
    vad_buf: Vec<i16>,
    /// Rolling audio cache used to restore the beginning of an utterance.
    preroll_frames: VecDeque<Vec<i16>>,
}
...
impl SilenceSuppressor {
    pub fn new(config: SilenceSuppressionConfig) -> Self {
        ...
        Self {
            ...
            preroll_frames: VecDeque::with_capacity(config.preroll_chunks),
            decimation_factor,
            vad,
            config,
    ...
    pub fn process(&mut self, frame: &[i16]) -> (FrameAction, bool) {
        ...
        if has_speech {
            let was_suppressed = self.state == SuppressionState::Suppressed && !self.was_speaking;
            self.state = SuppressionState::Active;
            self.last_speech_time = now;
            self.was_speaking = true;

            if was_suppressed && !self.preroll_frames.is_empty() {
                let preroll_len = self.preroll_frames.len();
                let mut combined = Vec::with_capacity((preroll_len + 1) * frame.len());
                while let Some(preroll) = self.preroll_frames.pop_front() {
                    combined.extend_from_slice(&preroll);
                }
                combined.extend_from_slice(frame);
                self.frames_sent += (preroll_len + 1) as u64;
                return (FrameAction::Send(combined), false);
            }
    ...
    #[inline]
    fn remember_preroll(&mut self, frame: &[i16]) {
        if self.config.preroll_chunks == 0 {
            return;
        }

        while self.preroll_frames.len() >= self.config.preroll_chunks {
            self.preroll_frames.pop_front();
        }
        self.preroll_frames.push_back(frame.to_vec());
    }
