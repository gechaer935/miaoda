 ...
 * `llmHelper.groqClient` seam so the REAL 秒答 backend (routing → context →
 ...
 * NO MiniMax transport — MiniMax lives only in the miaoda-api gateway. This adapter
...
// ── status-code → reason (mirrors miaoda-api/lib/minimaxProvider.js) ───────────
