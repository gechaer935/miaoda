...
function analyze() {
  ...
  for (const m of MODES) {
    ...
    byMode[m] = {
      ...
      identityLeaks: set.filter((r) => (r.failure_reason || '').includes('miaoda_identity') || (r.failure_reason || '').includes("forbidden_substring:I'm 秒答") || (r.failure_reason || '').includes('forbidden_substring:I am 秒答')).length,
  ...
  const critical = {
    identityLeak: clean.filter((r) => has(r, 'miaoda_identity') || has(r, "forbidden_substring:I'm 秒答") || has(r, 'forbidden_substring:I am 秒答')).length,
