/**
 * real-backend-intelligence-eval.mjs — drives the REAL 秒答 answer path
...
const MIAODA_LEAK_RE = /\bI(?:'m| am)\s+秒答|\bI(?:'m| am)\s+an?\s+(?:AI\s+)?(?:assistant|language model|chatbot)\b|\bas\s+an\s+AI(?:\s+(?:language\s+)?model)?,?\s+I\b/i;
...
const BOT_MARKERS = [
  ...
  { re: /\bI(?:'m| am) 秒答/i, tag: 'i_am_natively' },
...
function score(c, answer, plan, tokens) {
  ...
  for (const s of c.mustNotContain || []) {
    ...
    const hit = /^nativel?y$/i.test(needle.trim()) ? PRODUCT_MIAODA_RE.test(text) : lower.includes(needle.toLowerCase());
  ...
  if (c.expectedVoice === 'first_person_candidate' && MIAODA_LEAK_RE.test(text)) f.push('miaoda_identity_leak');
  ...
  // coding/technical profile-forbidden answers must not leak 秒答/profile —
  ...
  if (plan.profileContextPolicy === 'forbidden' && !c.profileShouldBeUsed) {
    ...
    if ((PRODUCT_MIAODA_RE.test(text) || CODING_PROFILE_RE_NONPRODUCT.test(text)) && !isInputRequest) f.push('coding_profile_leak');
...
function writeOutputs(raw, results, served, rotating, wallMs) {
  ...
  const meta = {
    ...
    leaks: { identity: cnt('miaoda_identity'), falseRefusal: cnt('false_refusal'), stealth: cnt('stealth_evasion'), codingProfile: cnt('coding_profile'), contextLeak: cnt('context_leak'), invented: cnt('invented_link'), hallucinated: cnt('hallucinated_source'), safetyNotRouted: cnt('safety_not_routed') },
