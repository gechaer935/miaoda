// tests/intelligence/e2e/MiaodaIntelligence100Questions.test.mjs
//
// ============================================================================
//  秒答 INTELLIGENCE OS — 100-QUESTION END-TO-END VERIFICATION SUITE
// ============================================================================
//
...
//      (real provider) against a SAFE COPY of the real miaoda.db. This is the EXACT
...
//
// Run: node --test tests/intelligence/e2e/MiaodaIntelligence100Questions.test.mjs
...
const ENABLED_FLAGS = {
  MIAODA_CONVERSATION_MEMORY_V2: '1',
  MIAODA_MEETING_MEMORY_V2: '1',
  MIAODA_GLOBAL_SEARCH_V2: '1',
  MIAODA_IN_MEETING_SEARCH_V2: '1',
  MIAODA_LECTURE_INTELLIGENCE_V2: '1',
  MIAODA_DIAGRAM_INTELLIGENCE: '1',
  MIAODA_ANSWER_DIVERSITY_GUARD: '1',
  MIAODA_HINDSIGHT_MEMORY: '1',
};
...
after(() => {
  ...
  const outPath = path.join(REPO_ROOT, 'miaoda-intelligence-e2e-results.json');
  const byCat = {};
...
for (const q of QUESTIONS.filter((x) => x.category === 'A' || x.category === 'B')) {
  test(`[${q.id}] ${q.category} manual — ${q.question}`, async () => {
    ...
    else if (scan.nativelyIdentityLeak) reason = 'WRONG IDENTITY — answer claims to be 秒答/an AI assistant on a candidate question';
...
for (const q of QUESTIONS.filter((x) => x.category === 'C')) {
  test(`[${q.id}] C WTA — ${q.question}`, async () => {
    ...
    else if (scan.nativelyIdentityLeak) reason = 'WRONG IDENTITY — "I\'m 秒答" in live copilot';
