 ...
 *   - No-op when MIAODA_SKIP_NOTARIZE=1 is set (explicit escape hatch).
...
module.exports = async function notarizeHook(context) {
  ...
  if (process.env.MIAODA_SKIP_NOTARIZE === '1') {
    console.log('[notarize] MIAODA_SKIP_NOTARIZE=1 — skipping notarization.');
    return;
  }
