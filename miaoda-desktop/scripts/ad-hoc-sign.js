 ...
 * shows "CoreServices Helper" instead of "秒答 Helper".
 *
...
exports.default = async function (context) {
    if (process.platform === 'win32') {
        // Miaoda does not use Chromium WebGPU/Vulkan. Disable the feature in
        // main.ts and omit its software Vulkan fallback from the Windows
        // package. This keeps the official installer below the Gitee release
        // attachment limit without removing the Direct3D/ANGLE renderer used
        // by the UI and screen capture flow.
        for (const file of ['vk_swiftshader.dll', 'vk_swiftshader_icd.json', 'vulkan-1.dll']) {
            fs.rmSync(path.join(context.appOutDir, file), { force: true });
        }
        return;
    }
    ...
    // When CSC_LINK / CSC_NAME / MIAODA_SIGN_IDENTITY is present, electron-builder performs
    ...
    const hasRealIdentity = !!(
        process.env.MIAODA_PRODUCTION_SIGN === '1' || // set by electron-builder.signed.cjs
        process.env.CSC_LINK ||
        process.env.CSC_NAME ||
        process.env.MIAODA_SIGN_IDENTITY
    );
    if (hasRealIdentity) {
        console.log(
            '[Ad-Hoc Signing] Developer ID identity detected (CSC_LINK/CSC_NAME/MIAODA_SIGN_IDENTITY) — ' +
    ...
    // that cannot be fully verified without a real signing identity. Set MIAODA_ADHOC_HARDENED=1
    // to opt in when testing entitlement/permission behavior locally.
    const hardenedOpt = process.env.MIAODA_ADHOC_HARDENED === '1' ? '--options runtime ' : '';
