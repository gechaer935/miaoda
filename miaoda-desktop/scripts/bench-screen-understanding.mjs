...
process.env.MIAODA_TEST_USER_DATA = process.env.MIAODA_TEST_USER_DATA || os.tmpdir();
...
// Build synthetic screenshots representative of the workloads 秒答 sees.
...
async function buildFixtures() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'miaoda-screen-bench-'));
  const fixtures = [];
