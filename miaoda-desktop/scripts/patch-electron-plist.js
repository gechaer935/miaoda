...
if (!content.includes('NSScreenCaptureUsageDescription')) {
  content = content.replace(
    ...
    '<key>NSScreenCaptureUsageDescription</key>\n\t<string>秒答需要「屏幕录制」权限，以便采集系统音频用于会议转写。</string>\n\t<key>NSMicrophoneUsageDescription</key>'
  );
  modified = true;
...
if (!content.includes('NSAudioCaptureUsageDescription')) {
  content = content.replace(
    '<key>NSMicrophoneUsageDescription</key>',
    '<key>NSAudioCaptureUsageDescription</key>\n\t<string>秒答需要系统音频权限，以便转写会议声音。</string>\n\t<key>NSMicrophoneUsageDescription</key>'
  );
  modified = true;
...
// The app then renames itself to "秒答" (app.setName + CFBundleName),
...
// promotion at startup paints exactly one correctly-timed "秒答" tile.
//
...
if (content.includes('This app needs access to the microphone')) {
  content = content.replace(
    ...
    '<string>秒答需要麦克风权限，以便在会议中转写你的语音。</string>'
  );
  modified = true;
