 ...
 *        "The signature of the binary is invalid" @ the packaged .app binary
 ...
 * (APPLE_KEYCHAIN_PROFILE, e.g. `miaoda-notary` — the existing local keychain profile name). No-op if none are present.
 */
...
const VOLICON = path.resolve(__dirname, '..', 'assets', 'miaoda.icns');
...
function resolveDeveloperIdIdentity() {
  if (process.env.MIAODA_SIGN_IDENTITY) return process.env.MIAODA_SIGN_IDENTITY;
...
function buildStyledDmg({ appPath, outDmg, identity }) {
  ...
  // exactly [the .app, Applications-droplink] and nothing stray.
  const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'miaoda-dmg-'));
